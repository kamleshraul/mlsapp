...
public class AssemblyRepository extends BaseRepository<Assembly, Long> {
     ...
     */
    public void updatePreviousCurrentAssembly(final Assembly assembly, final String locale) {
        Search search = new Search();
        search.addFilterEqual("currentAssembly", true);
        search.addFilterEqual("locale", locale);
        Assembly assembly1 = this.searchUnique(search);
        Long id = assembly.getId();
        if(id == null) {
            assembly1.setCurrentAssembly(false);
            assembly1.merge();
        } else {
        if( !(assembly.getId().equals(assembly1.getId()))) {
            assembly1.setCurrentAssembly(false);
            assembly1.merge();
        } }
    }
