...
@Repository
public class CommitteeMemberAttendanceRepository extends BaseRepository<MemberBallotAttendance, Serializable>{

	public String attendanceWithAllMembers(final CommitteeMeeting committeeMeeting,final String locale) throws ELSException {
		List<CommitteeMember> committeemembers=new ArrayList<CommitteeMember>();
		committeemembers=committeeMeeting.getCommittee().getMembers();
		List<CommitteeMember> committeeinvitedmembers=new ArrayList<CommitteeMember>();
		committeeinvitedmembers=committeeMeeting.getCommittee().getInvitedMembers();
		if(committeeMeeting.getCommitteeTour() == null)
		{
		
			for(CommitteeMember i:committeemembers){
				CommitteeMemberAttendance committeeMemberAttendance=null;
				committeeMemberAttendance=new CommitteeMemberAttendance();
				committeeMemberAttendance.setAttendance(false);
				committeeMemberAttendance.setCommitteeMeeting(committeeMeeting);
				committeeMemberAttendance.setCommitteeMember(i);
				committeeMemberAttendance.setLocale(locale);
				committeeMemberAttendance.persist();
			}	
	
			for(CommitteeMember i:committeeinvitedmembers){
				CommitteeMemberAttendance committeeMemberAttendance=null;
				committeeMemberAttendance=new CommitteeMemberAttendance();
				committeeMemberAttendance.setAttendance(false);
				committeeMemberAttendance.setCommitteeMeeting(committeeMeeting);
		...
		{
			List<TourItinerary> tourItineraries=new ArrayList<TourItinerary>();
			tourItineraries=committeeMeeting.getCommitteeTour().getItineraries();
			for(TourItinerary j:tourItineraries){
				for(CommitteeMember i:committeemembers){
					CommitteeMemberAttendance committeeMemberAttendance=null;
					committeeMemberAttendance=new CommitteeMemberAttendance();
					committeeMemberAttendance.setAttendance(false);
					...
					committeeMemberAttendance.setCommitteeMember(i);
					committeeMemberAttendance.setLocale(locale);
					committeeMemberAttendance.setTourItinerary(j);
					committeeMemberAttendance.persist();
				}	
				for(CommitteeMember i:committeeinvitedmembers){
					CommitteeMemberAttendance committeeMemberAttendance=null;
	...
	public List<CommitteeMemberAttendance> findAll (
			final TourItinerary tourItinerary,final String locale) throws ELSException {
		StringBuffer strQuery;
	
			strQuery = new StringBuffer(
					"SELECT m FROM CommitteeMemberAttendance m" +
							" WHERE m.tourItinerary.id=:tourItineraryId"+
							" AND m.locale=:locale"
					);
		
		try{
			TypedQuery<CommitteeMemberAttendance> jpQuery = this.em().createQuery(strQuery.toString(), CommitteeMemberAttendance.class);
			jpQuery.setParameter("tourItineraryId", tourItinerary.getId());	
			...
			elsException.setParameter("CommitteeMemberAttendanceRepository_List<CommitteeMemberAttendance>_findAll", "No attendance found.");
			throw elsException;
		}
	}
	
	public List<CommitteeMemberAttendance> findAll(final CommitteeMeeting committeeMeeting,
			final TourItinerary tourItinerary,final Boolean attendance,final String locale) throws ELSException {
		StringBuffer strQuery;
		if(tourItinerary == null)
		{
			 strQuery = new StringBuffer(
					"SELECT m FROM CommitteeMemberAttendance m" +
							" WHERE m.committeeMeeting.id=:committeeMeetingId" +
							" AND m.tourItinerary.id is null"+
		...
		else
		{
			strQuery = new StringBuffer(
					"SELECT m FROM CommitteeMemberAttendance m" +
							" WHERE m.committeeMeeting.id=:committeeMeetingId" +
							" AND m.tourItinerary.id=:tourItineraryId"+
							" AND m.attendance=:attendance"+
							" AND m.locale=:locale"
					);
		...
		try{
			TypedQuery<CommitteeMemberAttendance> jpQuery = this.em().createQuery(strQuery.toString(), CommitteeMemberAttendance.class);
			jpQuery.setParameter("committeeMeetingId", committeeMeeting.getId());
	...
	public List<Member> findMembersByAttendance(final Session session,
		...
		String query="SELECT m FROM CommitteeMemberAttendance mba" +
				" JOIN mba.member m" +
		...
		try{
			...
			logger.error(e.getMessage());
			ELSException elsException = new ELSException();
			elsException.setParameter("CommitteeMemberAttendanceRepository_List<Member>_findMembersByAttendance", "No member found.");
			throw elsException;
		}
	}
	...
	public List<Member> findEligibleMembers(final Session session,
		 ...
		 */
		String query="SELECT DISTINCT(m) FROM CommitteeMemberAttendance mba" +
				" JOIN mba.member m" +
				" WHERE mba.session.id=:sessionId" +
				" AND mba.deviceType.id=:deviceTypeId" +
		...
		try{
			...
			logger.error(e.getMessage());
			ELSException elsException = new ELSException();
			elsException.setParameter("CommitteeMemberAttendanceRepository_List<Member>_findEligibleMembers", "No member found.");
			throw elsException;
		}
	}
	 ...
	 * @return the boolean
	 */
	public String createCommitteeMemberAttendance(
	 ...
	 * @throws ELSException 
	 */
	public Boolean memberAttendanceCreated(final CommitteeMeeting committeeMeeting,
			final String locale) throws ELSException {
		/**** we are checking if attendance has already been created i.e entries are already
		 * present in CommitteeMemberAttendance for particular locale,devicetype,session and round****/

		String query="SELECT count(m.id) FROM CommitteeMemberAttendance m" +
				" WHERE m.committeeMeeting.id=:committeeMeetingId" +
					" AND m.locale=:locale";
...
//			CommitteeMemberAttendance committeeMemberAttendance=null;
//			if(round==1){
//				committeeMemberAttendance=new CommitteeMemberAttendance(session,questionType,i,false,round,false,locale);
//			}else{
//				CommitteeMemberAttendance previousRoundEntry=CommitteeMemberAttendance.find(session,questionType,i,round-1,locale);
//				committeeMemberAttendance=new CommitteeMemberAttendance(session,questionType,i,previousRoundEntry.getAttendance(),
//						round,false,locale);
//				committeeMemberAttendance.setPosition(previousRoundEntry.getPosition());
//			}			
//			committeeMemberAttendance.setCreatedBy(createdBy);
//			committeeMemberAttendance.setCreatedAs(createdAs);
//			committeeMemberAttendance.setEditedBy(createdBy);
//			committeeMemberAttendance.setEditedAs(createdAs);
//			Date date=new Date();
//			committeeMemberAttendance.setCreatedOn(date);
//			committeeMemberAttendance.setEditedOn(date);
//			committeeMemberAttendance.persist();
//		}		
//		return "ATTENDANCE_CREATION_SUCCESS";
//	}
