...
public class GenericController<T extends BaseDomain> extends BaseController {
    ...
    public String index(
        ...
        // messagePattern=spring message code starting pattern.It helps to follow some dynamic
        // pattern
     ...
     *
     * @param domain the domain
     * @param partialUpdate the partial update
     * @param id the id
     */
    @SuppressWarnings("unchecked")
    private void partialUpdate(final T domain, final String partialUpdate, final Long id) {
        ...
        if (!domain.getClass().getSuperclass().getSimpleName()
            ...
            int j = 0;
            for (int i = superClassFields.length;
                    i < (declaredFields.length + superClassFields.length); i++) {
    ...
    protected <E extends BaseDomain> void customInitBinder(final Class clazz,
        ...
        if (this.getUserLocale().equals(new Locale("mr", "IN"))) {
            SimpleDateFormat dateFormat = new SimpleDateFormat(parameter.getValue(), new Locale(
                    "hi", "IN"));
    ...
    /**
     * Custom init binder super class.
     *
     * @param <E> the element type
     * @param clazz the clazz
     * @param binder the binder
