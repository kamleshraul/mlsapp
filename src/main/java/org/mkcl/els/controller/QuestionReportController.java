...
//			currentDateCalendar.set(2015, 6, 12);
	...
	@RequestMapping(value="/ahwalRulesSuspensionReport" ,method=RequestMethod.GET)
	public @ResponseBody void generateAhwalRulesSuspensionReport(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
				...
				try {
					...
					if(houseType==null || sessionType==null) {
						...
						if(session==null) {								
							...
							queryParameters.put("questionType", new String[]{questionType});
							queryParameters.put("locale", new String[]{locale.toString()});
							String queryName= "QIS_AHWAL_RULES_SUSPENSION_REPORT";													
							List reportData = Query.findReport(queryName, queryParameters);
							if(reportData!=null && !reportData.isEmpty()) {
								List<String> serialNumbers = new ArrayList<String>();
								for(int i=1; i<=reportData.size(); i++) {
								...
								/**** generate report ****/
								if(!isError) {			
									if(questionType.equals(ApplicationConstants.DEVICE_RULESUSPENSIONMOTIONS)) {
										reportFile = generateReportUsingFOP(new Object[]{reportData,serialNumbers,localisedContent,questionType}, "qis_ahwal_rulesSuspension_"+houseType.getType(), "WORD", "qis_ahwal_rulesSuspension_Report", locale.toString());
									} else {
										reportFile = generateReportUsingFOP(new Object[]{reportData,serialNumbers,localisedContent,questionType}, "qis_ahwal_rulesSuspension_"+houseType.getType(), "WORD", "qis_ahwal_rulesSuspension_Report", locale.toString());
									}									
									if(reportFile!=null) {
				...
				try {
					...
					if(houseType==null || sessionType==null) {
						...
						if(session==null) {								
							...
							queryParameters.put("questionType", new String[]{questionType});
							queryParameters.put("locale", new String[]{locale.toString()});
							String queryName= "QIS_UNSTARRED_DEPARTMENTWISE_YAADI_UPPERHOUSE_REPORT";													
							List reportData = Query.findReport(queryName, queryParameters);
							if(reportData!=null && !reportData.isEmpty()) {
								List<String> serialNumbers = new ArrayList<String>();
								for(int i=1; i<=reportData.size(); i++) {
								...
								/**** generate report ****/
								if(!isError) {			
									if(questionType.equals(ApplicationConstants.DEVICE_QUESTIONS+"unstarred")) {
										reportFile = generateReportUsingFOP(new Object[]{reportData,serialNumbers,localisedContent,questionType}, "qis_unstarred_departmentwise_yaadi_"+houseType.getType()+"_report", "WORD", "qis_unstarred_departmentwise_yaadi_report", locale.toString());
									} else {
										reportFile = generateReportUsingFOP(new Object[]{reportData,serialNumbers,localisedContent,questionType}, "qis_unstarred_departmentwise_yaadi_"+houseType.getType()+"_report", "WORD", "qis_unstarred_departmentwise_yaadi_report", locale.toString());
									}									
									if(reportFile!=null) {
