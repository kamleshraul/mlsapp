	...
	public static void populateModule(final ModelMap model, 
		...
		if(userGroups != null && ! userGroups.isEmpty()) {
			...
			if(cp != null) {
				...
				throw new ELSException("ShortNoticeController.populateModule/4", 
...
//			"User group or User group type is not set for Username: " + currentUser.getUsername());
		...
		/**** Controlling Grids Starts ****/
		 ...
		 if(role != null){
			 ...
			 if(assistantGridAllowedFor != null){
				 ...
				 throw new ELSException("ShortNoticeController.modifyURLPattern/4", 
	...
	public static void populateNew(final ModelMap model,
		...
		if(selectedSession != null){
			...
			throw new ELSException("ShortNoticeController.populatenew/5", 
					"session_isnull");
		}
	...
	public static void preValidateCreate(final Question domain,
			final BindingResult result,
			final HttpServletRequest request,
	...
	public static void populateCreateIfNoErrors(final Question domain,
		...
		String operation = request.getParameter("operation");
		//set Edited As
		UserGroupType userGroupType = QuestionController.
				getUserGroupType(request,domain.getLocale());
		...
		if(domain.getHouseType()!=  null && domain.getSession()!= null &&  domain.getType() != null 
			...
			if(operation != null && !operation.isEmpty() 
				...
				Status newstatus = Status.findByType(ApplicationConstants.QUESTION_SHORTNOTICE_SUBMIT, domain.getLocale());
				...
				//set internalstatus, recommendation status, status
				Status status = Status.findByType(ApplicationConstants.QUESTION_SHORTNOTICE_COMPLETE, domain.getLocale());
			...
			//set internalstatus, recommendation status, status
			Status status = Status.findByType(ApplicationConstants.QUESTION_SHORTNOTICE_INCOMPLETE, domain.getLocale());
			domain.setStatus(status);
			domain.setInternalStatus(status);
			domain.setRecommendationStatus(status);
		}
	...
	public static void populateEdit(final Question domain, 
		...
		if(selectedSession == null){
			throw new ELSException("ShortNoticeController.populateEdit/4", 
					"Session is Null");
		}
		model.addAttribute("session", selectedSession.getId());
		...
		if(domain.getNumber()!=null){
			model.addAttribute("formattedNumber",FormaterUtil.
					getNumberFormatterNoGrouping(locale).
		...
		if(internalStatus != null){
			...
			if(usergroupType !=null && !(usergroupType.isEmpty()) && usergroupType.equals("assistant")){
				...
				if(!internalStatus.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_SUBMIT)){
					if(recommendationStatus.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_PUTUP_CLUBBING_POST_ADMISSION)
							|| recommendationStatus.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_PUTUP_CLUBBING_WITH_UNSTARRED_FROM_PREVIOUS_SESSION)
							|| recommendationStatus.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_PUTUP_UNCLUBBING)
							|| recommendationStatus.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_PUTUP_ADMIT_DUE_TO_REVERSE_CLUBBING)) {
						QuestionController.
...
//					populateInternalStatus(model,internalStatus.getType(),usergroupType,locale,deviceType.getType());
		...
		if(clubbedReferencedEntitiesVisibleUserGroups != null){
			...
			Boolean isUserGroupAllowed = QuestionController.
					isUserGroupTypeExists(userGroupTypes, userGroupType);
			//populate referencedEntities
			if(isUserGroupAllowed){
				List<Reference> refentities = QuestionController.
						getReferencedEntityReferences(domain,locale);
		...
		//Populate Supporting Member Names
		String supportingMemberNames = QuestionController.
				getDelimitedSupportingMembers(model, domain, usergroupType);
		model.addAttribute("supportingMembersName", supportingMemberNames);
		...
		if(domain.getInternalStatus() != null){
			...
			if(usergroupType!=null && !usergroupType.isEmpty() && usergroupType.equals("assistant")
					&&(internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_ADMISSION)
						||internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_DEPARTMENT)
						||internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_GOVT)
						||internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_MEMBER)
						||internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_MEMBER_AND_DEPARTMENT)
						||internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_REJECTION)
						||internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_PUTUP_REJECTION))){
		...
		Status tempStatus = Status.findByType(ApplicationConstants.QUESTION_SHORTNOTICE_FINAL_REJECTION, locale);
	...
	public static void customValidateUpdate(Question domain,
			BindingResult result, HttpServletRequest request, AuthUser authUser) {
		String role = request.getParameter("role");
...
//					populateInternalStatus(model,internalStatus.getType(),usergroupType,locale,deviceType.getType());
	...
	public static void populateUpdateIfNoErrors(final Question domain,
		...
		if(domain.getHouseType() != null && domain.getType() != null && domain.getSession() != null
			...
			if(operation != null && !operation.isEmpty()){
				...
				if(operation.trim().equals("submit")){
					...
					Status newstatus = Status.
							findByType(ApplicationConstants.QUESTION_SHORTNOTICE_SUBMIT, domain.getLocale());
					domain.setStatus(newstatus);
					domain.setInternalStatus(newstatus);
					domain.setRecommendationStatus(newstatus);
			...
			// operation is Null OR Empty
				...
				if(!userGroupType.getType().equals(ApplicationConstants.ASSISTANT)
					...
					Status status = Status.
							findByType(ApplicationConstants.QUESTION_SHORTNOTICE_COMPLETE, domain.getLocale());
					domain.setStatus(status);
					domain.setInternalStatus(status);
					domain.setRecommendationStatus(status);
				}
		...
		// Required Fields are not entered
			...
			// Set status, internalStatus, recommendationstatus
			Status status = Status.
					findByType(ApplicationConstants.QUESTION_SHORTNOTICE_INCOMPLETE, domain.getLocale());
			domain.setStatus(status);
			domain.setInternalStatus(status);
			domain.setRecommendationStatus(status);
		}
		...
		if(assistantProcessedAllowed != null){
			...
			if(isUserGroupAllowed){
				...
				if((internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_SUBMIT)
					||internalStatusType.equals(ApplicationConstants.QUESTION_SHORTNOTICE_SYSTEM_GROUPCHANGED)) 
				...
				if(group != null && draft != null) {
					...
					if(prevGroup != null && ! prevGroup.getNumber().equals(group.getNumber())) {
						Status GROUP_CHANGED = Status.
								findByType(ApplicationConstants.QUESTION_SHORTNOTICE_SYSTEM_GROUPCHANGED, domain.getLocale());
...
//			}
	...
	public static Boolean preDelete(final Question question,
		...
		if(question!=null){
			...
			if(status.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_INCOMPLETE)
					||status.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_COMPLETE)){
				return true;
			}else{
				return false;
			}
			...
			if(strStatus.equals("-")) {
				for(String i : selectedItems) {
					...
					if(!question.getInternalStatus().getType().
							equals(ApplicationConstants.QUESTION_SHORTNOTICE_SUBMIT)){
						/**** Create Process ****/
						...
						if(question.getInternalStatus().getType().equals(
							...
							recommendAdmission.append(question.formatNumber() + ",");
								...
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_REJECTION)){
							recommendRejection.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_REPEATREJECTION)){
							recommendRepeatRejection.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_REPEATADMISSION)){
							recommendRepeatAdmission.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_MEMBER)){
							recommendClarificationFromMember.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_DEPARTMENT)){
							recommendClarificationFromDept.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_GOVT)){
							recommendClarificationFromGovt.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLARIFICATION_FROM_MEMBER_AND_DEPARTMENT)){
							recommendClarificationFromMemberDept.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLUBBING)){
							recommendClubbing.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_NAMECLUBBING)){
							recommendNameClubbing.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLUBBING_POST_ADMISSION)){
							recommendClubbingPostAdmission.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_ADMIT_DUE_TO_REVERSE_CLUBBING)){
							recommendAdmitDueToReverseClubbing.append(question.formatNumber() + ",");
						}else if(question.getInternalStatus().getType().equals(
								ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_UNCLUBBING)){
					...
					if(actor != null && !actor.isEmpty()
						...
						try {
							...
							if(reference != null
								...
								if(!status.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_CLUBBING_POST_ADMISSION)
										&& !status.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_ADMIT_DUE_TO_REVERSE_CLUBBING)
										&& !status.getType().equals(ApplicationConstants.QUESTION_SHORTNOTICE_RECOMMEND_UNCLUBBING)) {
