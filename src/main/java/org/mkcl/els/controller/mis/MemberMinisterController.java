...
public class MemberMinisterController extends BaseController {
    ...
    public String newForm(final ModelMap model, final Locale locale,
        ...
        MemberMinisterAssociation domain = new MemberMinisterAssociation();
        populateNew(model, domain, locale.toString(), request);
        model.addAttribute("domain", domain);
        model.addAttribute("urlPattern", urlPattern);
        return "member/minister/new";
    }
    ...
    public String create(
            ...
            final Locale locale,
            @Valid @ModelAttribute("domain") final MemberMinisterAssociation domain,
        ...
        String returnUrl = "redirect:/member/minister/"
    ...
    public String update(
            final @Valid @ModelAttribute("domain") MemberMinisterAssociation domain,
    ...
    public String delete(final @PathVariable("recordIndex") int recordIndex,
            ...
            final HttpServletRequest request) {
    	MemberMinisterAssociation association = MemberMinisterAssociation
                .findByMemberIdAndId(member, recordIndex);
    ...
    protected void populateNew(final ModelMap model,
            final MemberMinisterAssociation domain, final String locale,
            final HttpServletRequest request) {
        List<Minister> memberMinister=Minister.findAll(Minister.class, "name", "asc", locale);
        model.addAttribute("ministers",memberMinister);
        Long member = Long.parseLong(request.getParameter("member"));
        model.addAttribute("member", member);
        int index = MemberMinisterAssociation.findHighestRecordIndex(member);
        domain.setRecordIndex(index + 1);
        domain.setLocale(locale.toString());
    }

    private void populateEdit(final ModelMap model,
            final MemberMinisterAssociation domain,
            final HttpServletRequest request, final String locale) {
    	 List<Minister> memberMinister=Minister.findAll(Minister.class, "name", "asc", locale);
         model.addAttribute("ministers",memberMinister);
         Long member=(Long)domain.getMember().getId();
       // Long member = (Long) request.getSession().getAttribute("member");
    ...
    private void poulateCreateIfErrors(final ModelMap model,
            final MemberMinisterAssociation domain, final HttpServletRequest request,
            final String locale) {
    ...
    private void validateCreate(final MemberMinisterAssociation domain,
            final Errors errors, final HttpServletRequest request) {
        if (domain.isDuplicate()) {
            Object[] params = new Object[4];
            params[1] = domain.getMinister().getName();
            ...
            errors.rejectValue("minister", "NonUnique",params,
                    "Duplicate Parameter");
                        
        }
    }

    private void validateUpdate(final MemberMinisterAssociation domain,
    ...
    private void initBinder(final WebDataBinder binder) {
        ...
        binder.registerCustomEditor(java.util.Date.class, new CustomDateEditor(
                dateFormat, true));
        binder.registerCustomEditor(Minister.class, new BaseEditor(
                new Minister()));
