...
public class NonCommiteeMembersController extends GenericController<NonCommiteeMember> {
	...
	@Override
	protected void populateList(final ModelMap model, final HttpServletRequest request, final String locale,
	...
	@Override
	protected void populateNew(final ModelMap model, final NonCommiteeMember domain, final String locale,
	...
	@Override
	protected void preValidateCreate(final NonCommiteeMember domain, final BindingResult result,
			final HttpServletRequest request) {
	...
	@Override
	protected void customValidateCreate(final NonCommiteeMember domain, final BindingResult result,
			final HttpServletRequest request) {

	}

	// =============== VALIDATIONS =========
	private void valEmptyAndNull(final NonCommiteeMember domain, final BindingResult result) {

		if (domain.getDepartmentName() == null || domain.getDepartmentName() == "-") {
			result.rejectValue("departmentName", "NotEmpty", "Department Name should not be empty");
		}

		if (domain.getNonCommitteeMemberType() == null) {
			result.rejectValue("nonCommitteeMemberType", "NotEmpty", "Non CommitteeMemberType should not be empty");
		}
	...
	@Override
	protected void populateEdit(final ModelMap model, final NonCommiteeMember domain,
			final HttpServletRequest request) {
...
//		String committeeMeetingId = request.getParameter("committeeMeetingId");
	...
	@Override
	protected void preValidateUpdate(final NonCommiteeMember domain, final BindingResult result,
			final HttpServletRequest request) {
		this.setCommitteeMeeting(domain, request);
		this.setNonCommitteeMemberType(domain, request);
		this.setNonCommiteeMemberInfos(domain, request);
	...
	private void preValidate(final NonCommiteeMember domain, final BindingResult result,
			final HttpServletRequest request) {
	}

	@Override
	protected void customValidateUpdate(final NonCommiteeMember domain, final BindingResult result,
			final HttpServletRequest request) {

	}

	@Override
	protected void populateAfterUpdate(final ModelMap model, final NonCommiteeMember domain,
			final HttpServletRequest request) {

	}

	@RequestMapping(value = "{id}/view", method = RequestMethod.GET)
	public String view(final ModelMap model, @PathVariable("id") final Long id, final Locale localeObj) {
	...
	public String deleteTourItinerary(final @PathVariable("id") Long id) {
		NonCommiteeMemberInformation nmInfo = NonCommiteeMemberInformation.findById(NonCommiteeMemberInformation.class,
				id);
		nmInfo.remove();
		return "info";
	}

	private void setNonCommiteeMemberInfos(final NonCommiteeMember domain, final HttpServletRequest request) {

		List<NonCommiteeMemberInformation> nonCommiteeMemberInformation = new ArrayList<NonCommiteeMemberInformation>();

		Integer noncommitteememberSize = Integer.parseInt(request.getParameter("noncommitteememberSize"));
		for (int i = 1; i <= noncommitteememberSize; i++) {
			...
			if (strnoncommitteemember != null && !strnoncommitteemember.isEmpty()) {
				...
				String strVersion = request.getParameter("prasInfoVersion" + i);
	...
	private void setCommitteeMeeting(final NonCommiteeMember domain, final HttpServletRequest request) {
	...
	private void setNonCommitteeMemberType(final NonCommiteeMember domain, final HttpServletRequest request) {
		String strnonCommitteeMemberTypes = request.getParameter("nonCommitteeMemberTypes");
		if (strnonCommitteeMemberTypes != null && !strnonCommitteeMemberTypes.isEmpty()) {
			Long nonCommitteeMemberTypeId = Long.parseLong(strnonCommitteeMemberTypes);
			NonCommitteeMemberType nonCommitteeMemberType = NonCommitteeMemberType
					.findById(NonCommitteeMemberType.class, nonCommitteeMemberTypeId);
			domain.setNonCommitteeMemberType(nonCommitteeMemberType);
		}
	}

	private List<NonCommitteeMemberType> populateNonCommitteeMemberTypes(final ModelMap model, final String locale) {
		List<NonCommitteeMemberType> nonCommitteeMemberTypes = NonCommitteeMemberType
				.findAll(NonCommitteeMemberType.class, "name", ApplicationConstants.ASC, locale);
	...
	private void commonPopulateEdit(final ModelMap model, final NonCommiteeMember domain,
			final HttpServletRequest request) {
		String locale = domain.getLocale();
