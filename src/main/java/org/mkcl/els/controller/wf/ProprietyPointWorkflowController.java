...
public class ProprietyPointWorkflowController  extends BaseController {
	...
	private void populateModel(final ProprietyPoint domain, final ModelMap model,
		...
		if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.SECTION_OFFICER)
				&& workflowDetails.getWorkflowSubType().equals(ApplicationConstants.PROPRIETYPOINT_FINAL_ADMISSION)
				&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.PROPRIETYPOINT_PROCESSED_SENDTOSECTIONOFFICER)) {
			
			String daysCountForReceivingReplyFromDepartment = "15";
		...
		if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.DEPARTMENT)
			...
			try{	
				...
				if(domain.getStatus().getType().equals(ApplicationConstants.PROPRIETYPOINT_FINAL_ADMISSION)) {
...
//				}
	...
	private void findLatestRemarksByUserGroup(final ProprietyPoint domain, final ModelMap model,
		...
		if(userGroupType.getType().equals(ApplicationConstants.DEPARTMENT)
			...
			List result=Query.findReport("PROIS_LATEST_REVISION_FOR_DESKOFFICER", requestMap);
	...
	public String updateMyTask(final ModelMap model,
		...
		try {
			...
			Ministry prevMinistry = proprietyPoint.getMinistry();
			SubDepartment prevSubDepartment = proprietyPoint.getSubDepartment();
			...
			if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.SECTION_OFFICER)
					&& workflowDetails.getWorkflowSubType().equals(ApplicationConstants.PROPRIETYPOINT_FINAL_ADMISSION)
					&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.PROPRIETYPOINT_PROCESSED_SENDTODEPARTMENT)
...
//				}
			...
			if(isMinistryChanged || isSubDepartmentChanged){
				Status sendToDepartmentStatus = Status.findByType(ApplicationConstants.PROPRIETYPOINT_PROCESSED_SENDTODEPARTMENT, locale.toString());
				if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.DEPARTMENT)
						|| workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.DEPARTMENT_DESKOFFICER)){
				...
				if(clubbedEntities!=null) {
					for(ClubbedEntity ce: clubbedEntities) {
						ProprietyPoint clubbedProprietyPoint = ce.getProprietyPoint();
						clubbedProprietyPoint.setMinistry(domain.getMinistry());
						clubbedProprietyPoint.setSubDepartment(domain.getSubDepartment());
						clubbedProprietyPoint.merge();
					}
				}			
				domain.merge();
				proprietyPoint = ProprietyPoint.findById(ProprietyPoint.class,domain.getId()); //added in order to avoid optimistic lock exception
				...
				Workflow workflow = Workflow.findByStatus(proprietyPoint.getInternalStatus(), locale.toString());
				//Motion in Post final status and pre ballot state can be group changed by Department 
				//as well as assistant of Secretariat
				WorkflowDetails.startProcessAtGivenLevel(proprietyPoint, ApplicationConstants.APPROVAL_WORKFLOW, workflow, ugt, assigneeLevel, locale.toString());
