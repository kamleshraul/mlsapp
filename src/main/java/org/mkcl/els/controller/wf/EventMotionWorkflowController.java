...
@Controller
@RequestMapping("/workflow/eventmotion")
public class EventMotionWorkflowController extends BaseController {
	...
	public String initSupportingMember(final ModelMap model,
		...
		EventMotion motion = EventMotion.findById(EventMotion.class, Long.parseLong(motionId));
		...
		if (member != null) {
			for (SupportingMember i : supportingMembers) {
				if (i.getMember().getId() == member.getId()) {
					i.setApprovedText(motion.getDescription());
					i.setApprovedSubject(motion.getEventTitle());
	...
	private void populateSupportingMember(final ModelMap model,
		...
		if(motion.getMember() != null){
			model.addAttribute("primaryMemberName", motion.getMember().getFullnameLastNameFirst());
		}
	}
	...
	public String initMyTask(final ModelMap model,
		...
		EventMotion domain = EventMotion.findById(EventMotion.class, Long.parseLong(workflowDetails.getDeviceId()));
		/**** Populate Model ****/
		populateModel(domain, model, request, workflowDetails);
		return workflowDetails.getForm();
	}

	private void populateModel(final EventMotion domain, 
		...
		if (userGroupId != null && !userGroupId.isEmpty()) {
			...
			if (userGroup.getUserGroupType().getType().equals("department")
					&& internalStatus.getType().equals(ApplicationConstants.EVENTMOTION_FINAL_ADMISSION)) {
				
				Status sendback = Status.findByType(ApplicationConstants.EVENTMOTION_RECOMMEND_SENDBACK, locale);
				actors = WorkflowConfig.findEventMotionActorsVO(domain, sendback, userGroup, Integer.parseInt(domain.getLevel()), locale);
			} else {
				actors = WorkflowConfig.findEventMotionActorsVO(domain, internalStatus, userGroup, Integer.parseInt(domain.getLevel()), locale);
			}
		...
		addDiscussionDates(domain, domain.getDeviceType(), model, selectedSession);
	...
	private void populateInternalStatus(final ModelMap model,
			final EventMotion domain, final String usergroupType, final String locale) {
		try {
	...
	public String updateMyTask(final ModelMap model,
			final HttpServletRequest request, final Locale locale,
			@Valid @ModelAttribute("domain") final EventMotion domain,
		...
		try{
			...
			if(domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_CLUBBING_POST_ADMISSION)
					|| domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_UNCLUBBING)
					|| domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_ADMIT_DUE_TO_REVERSE_CLUBBING)){
				
				workflowFromUpdatedStatus = Workflow.findByStatus(domain.getRecommendationStatus(), domain.getLocale());
			
			} else if(domain.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_CLUBBING)
					|| domain.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_NAME_CLUBBING)
					|| (domain.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)
						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_NOT_RECEIVED))
					||(domain.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)
						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_RECEIVED))
					||(domain.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER)
						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_NOT_RECEIVED))
					||(domain.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER)
						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_RECEIVED))) {
				...
				if (endflag != null) {
					if (!endflag.isEmpty()) {
						if (endflag.equals("continue")) {
							...
							if(domain.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER_DEPARTMENT)
									&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.EVENTMOTION_PROCESSED_SEND_TO_DEPARTMENT)){
		...
		return "workflow/eventmotion/" + userGroupType;
	}

	private void performAction(final EventMotion domain) {
		try{
			String internalStatus=domain.getInternalStatus().getType();
			String recommendationStatus=domain.getRecommendationStatus().getType();
			if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_ADMISSION)
					&&recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_ADMISSION)){
				performActionOnAdmission(domain);
			}
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECTION)
					&&recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECTION)){
				performActionOnRejection(domain);
			}
			/*** Clarification Asked  From Department***/
			else if(internalStatus.startsWith(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)
					&&recommendationStatus.startsWith(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)){
				performActionOnClarificationNeededFromDepartment(domain);
			}
			/*** Clarification Asked From Member***/
			else if(internalStatus.startsWith(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER)
					&&recommendationStatus.startsWith(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER)){
				performActionOnClarificationNeededFromMember(domain);
			}
			else if(internalStatus.startsWith(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER_DEPARTMENT)
					&&recommendationStatus.startsWith(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER_DEPARTMENT)){
				performActionOnClarificationNeededFromMemberAndDepartment(domain);
			}
			
			/**** Clarification not received From Department ****/
			else if(internalStatus.startsWith(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)
					&&recommendationStatus.startsWith(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_NOT_RECEIVED)){
			...
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER)
					&& recommendationStatus.equals(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_RECEIVED)){
				performActionOnClarificationReceived(domain);
			}
			/**** Clarification received From Department ****/
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)
					&& recommendationStatus.equals(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_RECEIVED)){
				performActionOnClarificationReceived(domain);
			}
			/**** Clarification received FROM Member & Department ****/
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER_DEPARTMENT)
					&& recommendationStatus.equals(ApplicationConstants.EVENTMOTION_PROCESSED_CLARIFICATION_RECEIVED)){
				performActionOnClarificationReceived(domain);
			}
			/**** Clubbing is approved ****/
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_CLUBBING)&&
					recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_CLUBBING)){
				performActionOnClubbing(domain);
			}
			/**** Clubbing is rejected ****/
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECT_CLUBBING)&&
					recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECT_CLUBBING)){
				performActionOnClubbingRejection(domain);
			}
			/**** Name clubbing is approved ****/
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_NAME_CLUBBING)&&
					recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_NAME_CLUBBING)){
				performActionOnNameClubbing(domain);
			}		
			/**** Name clubbing is rejected ****/		
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECT_NAME_CLUBBING)&&
					recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECT_NAME_CLUBBING)){
				performActionOnNameClubbingRejection(domain);
			}	
			/**** Clubbing Post Admission is approved ****/
			else if(recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_CLUBBING_POST_ADMISSION)){
				performActionOnClubbingPostAdmission(domain);
			}
			/**** Clubbing Post Admission is rejected ****/
			else if(recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECT_CLUBBING_POST_ADMISSION)){
				performActionOnClubbingRejectionPostAdmission(domain);
			}
			/**** Unclubbing is approved ****/
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_UNCLUBBING)&&
					recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_UNCLUBBING)){
				performActionOnUnclubbing(domain);
			}
			/**** Unclubbing is rejected ****/
			else if(internalStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECT_UNCLUBBING)&&
					recommendationStatus.equals(ApplicationConstants.EVENTMOTION_FINAL_REJECT_UNCLUBBING)){
	...
	private void performActionOnAdmission(final EventMotion domain) {
		Status finalStatus=Status.findByType(ApplicationConstants.EVENTMOTION_FINAL_ADMISSION, domain.getLocale());
		...
		if(domain.getRevisedEventTitle()==null){			
			domain.setRevisedEventTitle(domain.getEventTitle());			
		}else if(domain.getRevisedEventTitle().isEmpty()){
			domain.setRevisedEventTitle(domain.getEventTitle());
		}
		
		if(domain.getRevisedDescription()==null){			
			domain.setRevisedDescription(domain.getDescription());			
		}else if(domain.getRevisedDescription().isEmpty()){
			domain.setRevisedDescription(domain.getDescription());
		}		
		...
		if(clubbedEntities!=null){
			...
			if(domain.getRevisedEventTitle()!=null){
				if(!domain.getRevisedEventTitle().isEmpty()){
					subject=domain.getRevisedEventTitle();
				}else{
					subject=domain.getEventTitle();
				}
			}else{
				subject=domain.getEventTitle();
			}
			...
			if(domain.getRevisedDescription()!=null){
				if(!domain.getRevisedDescription().isEmpty()){
					details=domain.getRevisedDescription();
				}else{
					details=domain.getDescription();
				}
			}else{
				details=domain.getDescription();
			}
			
			Status status=Status.findByType(ApplicationConstants.EVENTMOTION_PUTUP_NAMECLUBBING, domain.getLocale());
			for(ClubbedEntity i:clubbedEntities){
				EventMotion motion=i.getEventMotion();
				if(motion.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_SYSTEM_CLUBBED)){
					motion.setRevisedEventTitle(subject);
					motion.setRevisedDescription(details);
	...
	private void performActionOnRejection(EventMotion domain) throws ELSException {
		...
		EventMotion.updateClubbing(domain);
		
	}
	
	private void performActionOnClarificationNeededFromDepartment(EventMotion domain) {
	...
	private void performActionOnClarificationNeededFromMember(EventMotion domain) {
		if(domain.getRevisedEventTitle() == null || domain.getRevisedEventTitle().isEmpty()){			
			domain.setRevisedEventTitle(domain.getEventTitle());			
		}
	...
	private void performActionOnClarificationNeededFromMemberAndDepartment(EventMotion domain) {
		if(domain.getRevisedEventTitle() == null || domain.getRevisedEventTitle().isEmpty()){			
			domain.setRevisedEventTitle(domain.getEventTitle());			
		}
	...
	private void performActionOnClarificationReceived(EventMotion domain) {
		Status newStatus = Status.findByType(ApplicationConstants.EVENTMOTION_SYSTEM_PUTUP, domain.getLocale());
	...
	private void performActionOnClubbing(EventMotion domain) throws ELSException {
	...
	private void performActionOnClubbingRejection(EventMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
		EventMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnNameClubbing(EventMotion domain) throws ELSException {
	...
	private void performActionOnNameClubbingRejection(EventMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
		EventMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnClubbingPostAdmission(EventMotion domain) throws ELSException {
	...
	private void performActionOnClubbingRejectionPostAdmission(EventMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
		EventMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnUnclubbing(EventMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
		EventMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnUnclubbingRejection(EventMotion domain) throws ELSException {
	...
	private void performActionOnClarificationNotReceived(EventMotion domain) {
		...
		try {
			...
			if (strHouseType != null && !(strHouseType.isEmpty())
				...
				if (strWorkflowSubType.equals("request_to_supporting_member")) {
					...
					CustomParameter deviceTypeInternalStatusUsergroup = CustomParameter.findByName(CustomParameter.class, "EVENTMOTION_PUT_UP_OPTIONS_" + motionType.getType().toUpperCase() + "_" + internalStatus.getType().toUpperCase() + "_" + strUserGroupType.toUpperCase(), "");
					CustomParameter deviceTypeHouseTypeUsergroup = CustomParameter.findByName(CustomParameter.class, "EVENTMOTION_PUT_UP_OPTIONS_" + motionType.getType().toUpperCase() + "_" + houseType.getType().toUpperCase() + "_" + strUserGroupType.toUpperCase(), "");
					CustomParameter deviceTypeUsergroup = CustomParameter.findByName(CustomParameter.class, "EVENTMOTION_PUT_UP_OPTIONS_" + motionType.getType() .toUpperCase() + "_" + strUserGroupType.toUpperCase(), "");
					if (finalApprovingAuthority != null
							&& finalApprovingAuthority.getValue().contains(strUserGroupType)) {
						CustomParameter finalApprovingAuthorityStatus = CustomParameter.findByName(CustomParameter.class, "EVENTMOTION_PUT_UP_OPTIONS_" + strUserGroupType.toUpperCase(), "");
						if (finalApprovingAuthorityStatus != null) {
			...
			return "workflow/eventmotion/bulkapprovalinit";
		...
		if (selectedItems != null && (selectedItems.length > 0)
			...
			for (String i : selectedItems) {
				if (strWorkflowSubType.equals("request_to_supporting_member")) {
					...
					WorkflowDetails wfDetails = WorkflowDetails.findById(WorkflowDetails.class, id);
					EventMotion motion = EventMotion.findById(EventMotion.class, Long.parseLong(wfDetails.getDeviceId()));
					...
					if (actor != null && !actor.isEmpty() && level != null
						...
						if (reference != null && reference.getId() != null
							...
							if(status != null){
								if(!status.getType().equals(ApplicationConstants.EVENTMOTION_RECOMMEND_DISCUSS) 
								&& !status.getType().equals(ApplicationConstants.EVENTMOTION_RECOMMEND_SENDBACK)
								&& !status.getType().equals(ApplicationConstants.EVENTMOTION_PROCESSED_SEND_TO_DEPARTMENT)
								&& !status.getType().equals(ApplicationConstants.EVENTMOTION_PROCESSED_SEND_TO_SECTION_OFFICER)){
									motion.setInternalStatus(status);
								}
								motion.setRecommendationStatus(status);	
								motion.setEndFlag("continue");
							...
							} else if (motion.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_ADMISSION)) {
								admittedMsg.append(motion.formatNumber() + ",");
							} else if (motion.getInternalStatus().getType().equals(ApplicationConstants.EVENTMOTION_FINAL_REJECTION)) {
	...
	private void populateBulkApprovalView(final Model model,
		...
		if (strHouseType != null && !(strHouseType.isEmpty())
			...
			for (WorkflowDetails i : workflowDetails) {
				...
				EventMotion motion = EventMotion.findById(EventMotion.class, Long.parseLong(i.getDeviceId()));
				...
				if (i.getWorkflowSubType().equals("request_to_supporting_member")) {
					...
					if(motion.getMember() != null){
						bulkApprovalVO.setMember(motion.getMember().getFullname());
					}
					bulkApprovalVO.setSubject(motion.getEventTitle());
					List<SupportingMember> supportingMembers = motion.getSupportingMembers();
