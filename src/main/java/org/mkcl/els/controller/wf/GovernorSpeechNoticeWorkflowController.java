...
@Controller
@RequestMapping("/workflow/governorspeechnotice")
public class GovernorSpeechNoticeWorkflowController extends BaseController {
	...
	public String initMyTask(final ModelMap model,
		...
		try {
			...
			GovernorSpeechNotice domain=GovernorSpeechNotice.findById(GovernorSpeechNotice.class,Long.parseLong(workflowDetails.getDeviceId()));
			/**** Populate Model ****/
	...
	private void populateModel(final GovernorSpeechNotice domain, final ModelMap model,
		 ...
		 * "SMIS_REMARKS_FOR_DECISION_ALLOWED_FOR","");
		 * if(remarksForDecisionAllowed!=null) { List<UserGroupType> userGroupTypes =
		 ...
		 * domain.setRemarks(domain.getRemarksAboutDecision()); } else {
		 * domain.setRemarks(""); } } else { domain.setRemarks(""); }
		 */
		...
		/*
		 * Date currentDate = new Date(); if(currentDate.equals(rotationOrderPubDate) ||
...
//			}
		...
		/*
		 * if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.
		 * DEPARTMENT) ||
		 * workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.
		 * DEPARTMENT_DESKOFFICER)) {
		 ...
		 * else { model.addAttribute("formattedNumber",FormaterUtil.
		 * getNumberFormatterNoGrouping(locale).format(domain.getNumber())); } } else {
		 * if(domain.getNumber()!=null){
		 * model.addAttribute("formattedNumber",FormaterUtil.
		...
		/*
		 * if(selectedSession!=null && selectedSession.getId()!=null) { List<Date>
		 * sessionDates = selectedSession.findAllSessionDates();
		 * model.addAttribute("sessionDates",
		 * this.populateDateListUsingCustomParameterFormat(sessionDates,
		 ...
		 * if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.
		 * SECTION_OFFICER) &&
		 * workflowDetails.getWorkflowSubType().equals(ApplicationConstants.
		 * SPECIALMENTIONNOTICE_FINAL_ADMISSION) &&
		 * domain.getRecommendationStatus().getType().equals(ApplicationConstants.
		 * SPECIALMENTIONNOTICE_PROCESSED_SENDTOSECTIONOFFICER)) {
		 * 
		 * String daysCountForReceivingReplyFromDepartment = "2"; CustomParameter
		 * csptDaysCountForReceivingReplyFromDepartment =
		 * CustomParameter.findByName(CustomParameter.class,
		 * domain.getType().getType().toUpperCase()+"_"+houseType.getType().toUpperCase(
		 * )+"_"+ApplicationConstants.DAYS_COUNT_FOR_RECEIVING_ANSWER_FROM_DEPARTMENT,
		 * ""); if(csptDaysCountForReceivingReplyFromDepartment!=null &&
		 * csptDaysCountForReceivingReplyFromDepartment.getValue()!=null) {
		 * daysCountForReceivingReplyFromDepartment =
		 * csptDaysCountForReceivingReplyFromDepartment.getValue(); }
		 * if(domain.getReplyRequestedDate()!=null) { lastDateOfReplyReceiving =
		 * Holiday.getNextWorkingDateFrom(domain.getReplyRequestedDate(),
		 * Integer.parseInt(daysCountForReceivingReplyFromDepartment), locale); } else {
		 * lastDateOfReplyReceiving = Holiday.getNextWorkingDateFrom(new Date(),
		 * Integer.parseInt(daysCountForReceivingReplyFromDepartment), locale); }
		 * domain.setLastDateOfReplyReceiving(lastDateOfReplyReceiving); } else {
		 * if(domain.getLastDateOfReplyReceiving()!=null) { lastDateOfReplyReceiving =
		 * domain.getLastDateOfReplyReceiving(); } } if(lastDateOfReplyReceiving!=null)
		 * { model.addAttribute("lastDateOfReplyReceiving", lastDateOfReplyReceiving);
		 * model.addAttribute("formattedLastReplyReceivingDate",
		 * FormaterUtil.formatDateToString(lastDateOfReplyReceiving,
...
//		}else{
			actors = WorkflowConfig.findGovernorSpeechNoticeActorsVO(domain, internalStatus, userGroup, Integer.parseInt(domain.getLevel()), locale);
//		}
...
//		}
	...
	private void findLatestRemarksByUserGroup(final GovernorSpeechNotice domain, final ModelMap model,
		...
		if(userGroupType!=null) {
			customParameter = CustomParameter.findByName(CustomParameter.class, "GSN_LATESTREVISION_STARTINGACTOR_"+userGroupType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase(), "");
			if(customParameter != null){
				...
				CustomParameter defaultCustomParameter = CustomParameter.findByName(CustomParameter.class, "GSN_LATESTREVISION_STARTINGACTOR_DEFAULT"+"_"+houseType.getType().toUpperCase(), "");
		...
		requestMap.put("governorSpeechNoticeId",new String[]{String.valueOf(domain.getId())});
		requestMap.put("locale",new String[]{domain.getLocale()});
		List result=Query.findReport("GSN_LATEST_REVISIONS", requestMap);
...
//		}
	...
	public String updateMyTask(final ModelMap model,
			final HttpServletRequest request,
			final Locale locale,@Valid @ModelAttribute("domain") final GovernorSpeechNotice domain,final BindingResult result) {
		/**** Workflowdetails ****/
...
//			}
			...
			if(domain.getDrafts()==null) {
				//if(specialMentionNotice==null) {
					domain.setDrafts(governorSpeechNotice.getDrafts());
...
//								}
		...
		return "workflow/governorspeechnotice/"+userGroupType;				
	}	
...
//			String strAdjourningDate = request.getParameter("specialMentionNoticeDate");
			...
			if(userGroups != null && !userGroups.isEmpty()){
				CustomParameter customParameter = CustomParameter.
						findByName(CustomParameter.class,"GSN_ALLOWED_USERGROUPTYPES", "");
				if(customParameter != null){
...
//				}
			...
			if(strHouseType!=null&&!(strHouseType.isEmpty())
					...
					CustomParameter deviceTypeInternalStatusUsergroup = CustomParameter.
							findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+motionType.getType().toUpperCase()+"_"+internalStatus.getType().toUpperCase()+"_"+strUserGroupType.toUpperCase(), "");
					CustomParameter deviceTypeHouseTypeInternalStatusUsergroup = CustomParameter.
							findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+motionType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase()+"_"+internalStatus.getType().toUpperCase()+"_"+strUserGroupType.toUpperCase(), "");
					CustomParameter deviceTypeHouseTypeUsergroup = CustomParameter.
							findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+motionType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase()+"_"+strUserGroupType.toUpperCase(), "");
					CustomParameter deviceTypeUsergroup = CustomParameter.
							findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+motionType.getType().toUpperCase()+"_"+strUserGroupType.toUpperCase(), "");
					if(finalApprovingAuthority!=null 
						...
						CustomParameter finalApprovingAuthorityStatus = CustomParameter.
								findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+strUserGroupType.toUpperCase(),"");
					...
					}/**** GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_+DEVICETYPE_TYPE+INTERNALSTATUS_TYPE+USERGROUP(Post Final Status)****/
					else if(deviceTypeInternalStatusUsergroup != null){
						internalStatuses = Status.
								findStatusContainedIn(deviceTypeInternalStatusUsergroup.getValue(), strLocale);
					}/**** GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_+DEVICETYPE_TYPE + HOUSETYPE + INTERNALSTATUS_TYPE+USERGROUP(PRE Final Status)****/
					...
					/**** GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_+DEVICETYPE_TYPE+HOUSETYPE+USERGROUP(Pre Final Status-House Type Basis)****/
					else if(deviceTypeHouseTypeUsergroup != null){
						internalStatuses = Status.
								findStatusContainedIn(deviceTypeHouseTypeUsergroup.getValue(), strLocale);
					}	
					/**** GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_+DEVICETYPE_TYPE+USERGROUP(Pre Final Status)****/
...
//					}
					...
					List<WorkflowDetails> workflowDetails = WorkflowDetails.
							findAllForGovernorSpeechNotices(strHouseType, strSessionType, strSessionYear,
					...
					for(WorkflowDetails i:workflowDetails){
						...
						GovernorSpeechNotice motion=GovernorSpeechNotice.findById(GovernorSpeechNotice.class,Long.parseLong(i.getDeviceId()));
						{
...
//							}
			...
			return "workflow/governorspeechnotice/advancedbulkapproval";	
	...
	public String advancedBulkApproval(final HttpServletRequest request,
		...
		GovernorSpeechNotice tempMotion  = null;
		if(listSize != null && !listSize.isEmpty()){
			for(int i =0; i<Integer.parseInt(listSize);i++){
				...
				if(strChecked != null && !strChecked.isEmpty() && Boolean.parseBoolean(strChecked)){
					GovernorSpeechNotice motion = GovernorSpeechNotice.findById(GovernorSpeechNotice.class,Long.parseLong(wfDetails.getDeviceId()));
					tempMotion = motion;
...
//						motion.setRemarksAboutDecision(motion.getRemarks());
					...
					if(internalStatus!=null && !internalStatus.isEmpty()) {	//decision is selected meaning that statuses and flow should be updated accordingly
						...
						if(!intStatus.getType().equals(ApplicationConstants.GOVERNORSPEECHNOTICE_RECOMMEND_DISCUSS) 
								&& !intStatus.getType().equals(ApplicationConstants.GOVERNORSPEECHNOTICE_RECOMMEND_SENDBACK)
...
//						wfDetails.setAdjourningDate(motion.getSpecialMentionNoticeDate());
						...
						if(strUserGroup!=null) {
							...
							List<Reference> actors = WorkflowConfig.findGovernorSpeechNoticeActorsVO(motion,motion.getInternalStatus(),userGroup,Integer.parseInt(motion.getLevel()),locale.toString());
...
//		}
        ...
        String returnUrl = "redirect:/workflow/governorspeechnotice/advancedbulkapproval";
        return returnUrl;
//		getAdvancedBulkApproval(request, locale, model);
	}
	
	private void populateInternalStatus(final ModelMap model,
			final GovernorSpeechNotice domain, final Status putupOptionsStatus,
			final String locale) {
		try{
			List<Status> internalStatuses = new ArrayList<Status>();
			...
			if(actor==null){
				CustomParameter defaultStatus = CustomParameter.
						findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+deviceType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase()+"_DEFAULT", "");
				internalStatuses = Status.findStatusContainedIn(defaultStatus.getValue(), locale);
			}else if(actor.isEmpty()){
				CustomParameter defaultStatus = CustomParameter.findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+deviceType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase()+"_DEFAULT", "");
				internalStatuses = Status.findStatusContainedIn(defaultStatus.getValue(), locale);
			}else{
				...
				CustomParameter deviceTypeHouseTypeUsergroup = CustomParameter.findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+deviceType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase()+"_"+putupOptionsStatus.getType().toUpperCase()+"_"+usergroupType.toUpperCase(), "");
				CustomParameter deviceTypeInternalStatusUsergroup = CustomParameter.findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+deviceType.getType().toUpperCase()+"_"+putupOptionsStatus.getType().toUpperCase()+"_"+usergroupType.toUpperCase(), "");
				CustomParameter deviceTypeUsergroup = CustomParameter.findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+deviceType.getType().toUpperCase()+"_"+usergroupType.toUpperCase(), "");
				if(finalApprovingAuthority != null && finalApprovingAuthority.getValue().contains(usergroupType)) {
					...
					if(workflow!=null) {
						finalApprovingAuthorityStatus = CustomParameter.findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_" + deviceType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase()+"_"+workflow.getType().toUpperCase()+"_FINAL_AUTHORITY", "");
						if(finalApprovingAuthorityStatus==null) {
							finalApprovingAuthorityStatus = CustomParameter.findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_" + deviceType.getType().toUpperCase()+"_"+workflow.getType().toUpperCase()+"_FINAL_AUTHORITY", "");
						}						
					}
					if(finalApprovingAuthorityStatus == null) {
						finalApprovingAuthorityStatus = CustomParameter.findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_" + deviceType.getType().toUpperCase() + "_" + usergroupType.toUpperCase(), "");
					}
					if(finalApprovingAuthorityStatus == null) {
						finalApprovingAuthorityStatus=CustomParameter.findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+usergroupType.toUpperCase(),"");
					}
					if(finalApprovingAuthorityStatus != null){
						internalStatuses=Status.
	...
	private void performAction(final GovernorSpeechNotice domain, HttpServletRequest request) throws ELSException {
		String internalStatus=domain.getInternalStatus().getType();
		String recommendationStatus=domain.getRecommendationStatus().getType();
		/**** Admission ****/
		if(internalStatus.equals(ApplicationConstants.GOVERNORSPEECHNOTICE_FINAL_ADMISSION)
				&&recommendationStatus.equals(ApplicationConstants.GOVERNORSPEECHNOTICE_FINAL_ADMISSION)){
			performActionOnAdmission(domain, request);
		} 
		/**** Rejection ****/
		else if(internalStatus.equals(ApplicationConstants.GOVERNORSPEECHNOTICE_FINAL_REJECTION)
				&&recommendationStatus.equals(ApplicationConstants.GOVERNORSPEECHNOTICE_FINAL_REJECTION)){
...
//		}
	...
	private void performActionOnAdmission(GovernorSpeechNotice domain, HttpServletRequest request) throws ELSException {
...
//		SpecialMentionNotice.updateClubbing(domain);
	...
	private void performActionOnRejection(GovernorSpeechNotice domain, HttpServletRequest request) throws ELSException {
		domain.setStatus(domain.getInternalStatus());
		if(domain.getRevisedSubject() == null 
				||domain.getRevisedSubject().isEmpty()){			
			domain.setRevisedSubject( domain.getSubject());	
	...
	public String getBulkApprovalInit(final HttpServletRequest request,final Locale locale,
		...
		if(userGroups!=null){
			if(!userGroups.isEmpty()){
				CustomParameter customParameter=CustomParameter.findByName(CustomParameter.class,"GSN_ALLOWED_USERGROUPTYPES", "");
				if(customParameter!=null){
					String allowedUserGroups=customParameter.getValue(); 
					for(UserGroup i:userGroups){
						if(allowedUserGroups.contains(i.getUserGroupType().getType())){
		...
		if(strHouseType!=null&&!(strHouseType.isEmpty())
			...
			/**** List of Statuses ****/
				...
				CustomParameter deviceTypeInternalStatusUsergroup=CustomParameter.findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+motionType.getType().toUpperCase()+"_"+internalStatus.getType().toUpperCase()+"_"+strUserGroupType.toUpperCase(), "");
				CustomParameter deviceTypeHouseTypeUsergroup=CustomParameter.findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+motionType.getType().toUpperCase()+"_"+houseType.getType().toUpperCase()+"_"+strUserGroupType.toUpperCase(), "");
				CustomParameter deviceTypeUsergroup=CustomParameter.findByName(CustomParameter.class, "GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+motionType.getType().toUpperCase()+"_"+strUserGroupType.toUpperCase(), "");
				if(finalApprovingAuthority!=null&&finalApprovingAuthority.getValue().contains(strUserGroupType)){
					CustomParameter finalApprovingAuthorityStatus=CustomParameter.findByName(CustomParameter.class,"GOVERNORSPEECHNOTICE_PUT_UP_OPTIONS_"+strUserGroupType.toUpperCase(),"");
					if(finalApprovingAuthorityStatus!=null){
						try {
		...
		return "workflow/governorspeechnotice/bulkapprovalinit";		
	}	
	...
	public String bulkApproval(final HttpServletRequest request,final Locale locale,
		...
		if(selectedItems != null && (selectedItems.length >0)
			...
			for(String i : selectedItems) {
					...
					GovernorSpeechNotice governorSpeechNotice = GovernorSpeechNotice.findById(GovernorSpeechNotice.class,Long.parseLong(wfDetails.getDeviceId()));
					houseType=governorSpeechNotice.getHouseType();
					String actor=request.getParameter("actor");
					if(actor==null||actor.isEmpty()){
						actor=governorSpeechNotice.getActor();
						String[] temp=actor.split("#");
						actor=temp[1];
					}
					String level=request.getParameter("level");
					if(level==null||level.isEmpty()){
						level=governorSpeechNotice.getLevel();
					}
					if(actor!=null&&!actor.isEmpty()
							&&level!=null&&!level.isEmpty()){
						Reference reference=null;
						try {
							reference=UserGroup.findGovernorSpeechNoticeActor(governorSpeechNotice,actor,level,locale.toString());
						} catch (ELSException e) {
							e.printStackTrace();
							model.addAttribute("error", e.getParameter());
						}
						if(reference!=null
							...
							governorSpeechNotice.setActor(reference.getId());
							governorSpeechNotice.setLocalizedActorName(temp[3]+"("+temp[4]+")");
							governorSpeechNotice.setLevel(temp[2]);
							
							//**** Update Internal Status and Recommendation Status ****//
							if(status!=null){
								governorSpeechNotice.setInternalStatus(status);
								governorSpeechNotice.setRecommendationStatus(status);
							}
							String endFlag=null;
							governorSpeechNotice.setEndFlag("continue");
							endFlag=governorSpeechNotice.getEndFlag();
										...
										if(timerflag.equals("set")) {
											...
											if(reminderflag!=null) {
												if(reminderflag.equals("set")) {
													...
													String remindersubject=governorSpeechNotice.getSubject();
													properties.put("pv_remindersubject", remindersubject);
													
													//String remindercontent=request.getParameter("remindercontent");
													String remindercontent=governorSpeechNotice.getNoticeContent();
							...
							if(endFlag!=null&&!endFlag.isEmpty()
								...
								try {
									workflowDetails2 = WorkflowDetails.create(governorSpeechNotice,newtask,usergroupType,ApplicationConstants.GOVERNORSPEECHNOTICE_APPROVAL_WORKFLOW,level);
									governorSpeechNotice.setWorkflowDetailsId(workflowDetails2.getId());
							...
							//**** Update Old Workflow Details ****//
							wfDetails.setStatus("COMPLETED");
							wfDetails.setInternalStatus(governorSpeechNotice.getInternalStatus().getName());
							wfDetails.setRecommendationStatus(governorSpeechNotice.getRecommendationStatus().getName());;	

							wfDetails.setCompletionTime(new Date());
							wfDetails.merge();
							//**** Update Resolution ****//
							governorSpeechNotice.setEditedOn(new Date());
							governorSpeechNotice.setEditedBy(this.getCurrentUser().getActualUsername());
							governorSpeechNotice.setEditedAs(wfDetails.getAssigneeUserGroupName());	
							
							if(remark != null && !remark.isEmpty()){
								governorSpeechNotice.setRemarks(remark);
							}
													
							try {
								performAction(governorSpeechNotice, request);
							} catch (ELSException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
...
//							}
							governorSpeechNotice.setEditedOn(new Date());
							governorSpeechNotice.merge();
							internalStatus=governorSpeechNotice.getInternalStatus();	
							if(internalStatus.getType().equals(ApplicationConstants.GOVERNORSPEECHNOTICE_RECOMMEND_ADMISSION)){
								recommendAdmissionMsg.append(governorSpeechNotice.formatNumber()+",");
							}else if(internalStatus.getType().equals(ApplicationConstants.GOVERNORSPEECHNOTICE_RECOMMEND_REJECTION)){
								recommendRejectionMsg.append(governorSpeechNotice.formatNumber()+",");
							}else if(internalStatus.getType().equals(ApplicationConstants.GOVERNORSPEECHNOTICE_FINAL_ADMISSION)){
								admittedMsg.append(governorSpeechNotice.formatNumber()+",");
							}else if(internalStatus.getType().equals(ApplicationConstants.GOVERNORSPEECHNOTICE_FINAL_REJECTION)){
								rejectedMsg.append(governorSpeechNotice.formatNumber()+",");
						}
					}					
				}			
			}			
		...
		return "workflow/governorspeechnotice/bulkapprovalview";
	}
	...
	private void populateBulkApprovalView(final Model model,
		...
		if(strHouseType!=null&&!(strHouseType.isEmpty())
			...
			for(WorkflowDetails i:workflowDetails){
				BulkApprovalVO bulkApprovalVO=new BulkApprovalVO();				
				GovernorSpeechNotice governorSpeechNotice = GovernorSpeechNotice.findById(GovernorSpeechNotice.class,Long.parseLong(i.getDeviceId()));
				/**** Bulk Submission For Workflows ****/
					/**** Status Wise Bulk Submission ****/
					
						bulkApprovalVO.setId(String.valueOf(i.getId()));
						bulkApprovalVO.setDeviceId(String.valueOf(governorSpeechNotice.getId()));				
						if(governorSpeechNotice.getNumber()!=null){
							bulkApprovalVO.setDeviceNumber(format.format(governorSpeechNotice.getNumber()));
						}else{
							bulkApprovalVO.setDeviceNumber("-");
						}
...
//						}
						bulkApprovalVO.setDeviceType(governorSpeechNotice.getType().getName());
						bulkApprovalVO.setMember(governorSpeechNotice.getPrimaryMember().getFullname());
						if(governorSpeechNotice.getRevisedSubject() != null && !governorSpeechNotice.getRevisedSubject().isEmpty()){
							bulkApprovalVO.setSubject(governorSpeechNotice.getRevisedSubject());
						}else{
							bulkApprovalVO.setSubject(governorSpeechNotice.getSubject());
						}
						if(governorSpeechNotice.getRemarks()!=null&&!governorSpeechNotice.getRemarks().isEmpty()){
							bulkApprovalVO.setLastRemark(governorSpeechNotice.getRemarks());
						}else{
							bulkApprovalVO.setLastRemark("-");
						}
						bulkApprovalVO.setLastDecision(governorSpeechNotice.getInternalStatus().getName());	
						Map<String, String[]> requestMap=new HashMap<String, String[]>();			
						requestMap.put("resolutionId",new String[]{String.valueOf(governorSpeechNotice.getId())});
						requestMap.put("locale",new String[]{governorSpeechNotice.getLocale()});
						List result=Query.findReport("GSN_GET_REVISION", requestMap);
						bulkApprovalVO.setRevisions(result);
						bulkApprovalVO.setLastRemarkBy(governorSpeechNotice.getEditedAs());	
						bulkApprovalVO.setCurrentStatus(i.getStatus());
						bulkapprovals.add(bulkApprovalVO);
