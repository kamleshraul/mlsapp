...
@Controller
@RequestMapping("/workflow/appropriationbillmotion")
public class AppropriationBillMotionWorkflowController extends BaseController {
	...
	public String initSupportingMember(final ModelMap model,
		...
		AppropriationBillMotion motion = AppropriationBillMotion.findById(AppropriationBillMotion.class, Long.parseLong(motionId));
	...
	private void populateSupportingMember(final ModelMap model,
			final AppropriationBillMotion motion,
		...
		if(motion.getAmountDemanded() != null){
			model.addAttribute("formattedAmountDemanded",FormaterUtil.formatNumberForIndianCurrencyWithSymbol(motion.getAmountDemanded(), locale));
		}
		/**** Decision Status ****/
	...
	public String initMyTask(final ModelMap model,
		...
		AppropriationBillMotion domain = AppropriationBillMotion.findById(AppropriationBillMotion.class, Long.parseLong(workflowDetails.getDeviceId()));
		/**** Populate Model ****/
	...
	private void populateModel(final AppropriationBillMotion domain, final ModelMap model,
		...
		/**** Subset of Ministries for the department of the current appropriationbillmotion (to facilitate internal ministry changes without transfer in department user login only) ****/
		...
		if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.SECTION_OFFICER)
				&& workflowDetails.getWorkflowSubType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION)
				&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_SENDTOSECTIONOFFICER)) {
		...
		if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.DEPARTMENT)
			...
			try{	
				...
				if(domain.getStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION)) {
...
//		}
		...
		if (userGroupId != null && !userGroupId.isEmpty()) {
			...
			if (userGroup.getUserGroupType().getType().equals("department")
					&& internalStatus.getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION)) {
				
				Status sendback = Status.findByType(ApplicationConstants.APPROPRIATIONBILLMOTION_RECOMMEND_SENDBACK, locale);
				actors = WorkflowConfig.findAppropriationBillMotionActorsVO(domain, sendback, userGroup, Integer.parseInt(domain.getLevel()), locale);
				
			} else if (!userGroup.getUserGroupType().getType().equals("member")) {
				actors = WorkflowConfig.findAppropriationBillMotionActorsVO(domain, internalStatus, userGroup, Integer.parseInt(domain.getLevel()), locale);
			}
	...
	private void populateInternalStatus(final ModelMap model,
			final AppropriationBillMotion domain, final String usergroupType, final String locale) {
		try {
			...
			if(internalStatuses!=null && !internalStatuses.isEmpty()) {
				for(Status ista: internalStatuses) {
					if(ista.getType().trim().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_REPLY_RECEIVED)
	...
	public String updateMyTask(final ModelMap model,
			final HttpServletRequest request, final Locale locale,
			@Valid @ModelAttribute("domain") final AppropriationBillMotion domain,
...
//			}
			...
			userGroupType = workflowDetails.getAssigneeUserGroupType();
			AppropriationBillMotion appropriationBillMotion = AppropriationBillMotion.findById(AppropriationBillMotion.class, domain.getId());
			Ministry prevMinistry = appropriationBillMotion.getMinistry();
			SubDepartment prevSubDepartment = appropriationBillMotion.getSubDepartment();
			/**** Updating domain ****/
			...
			if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.SECTION_OFFICER)
					&& workflowDetails.getWorkflowSubType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION)
					&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_SENDTODEPARTMENT)
					&& (domain.getReply()==null || domain.getReply().isEmpty())) {
				if(strReplyRequestedDate == null || strReplyRequestedDate.isEmpty()) {
...
//			if(domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_CLUBBING_POST_ADMISSION)
//					|| domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_UNCLUBBING)
//					|| domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMIT_DUE_TO_REVERSE_CLUBBING)){
//				
//				workflowFromUpdatedStatus = Workflow.findByStatus(domain.getRecommendationStatus(), domain.getLocale());
//			
//			} else if(domain.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_CLUBBING)
//					|| domain.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_NAME_CLUBBING)
//					|| (domain.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)
//						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_CLARIFICATION_NOT_RECEIVED))
//					||(domain.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_CLARIFICATION_NEEDED_FROM_DEPARTMENT)
//						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_CLARIFICATION_RECEIVED))
//					||(domain.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER)
//						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_CLARIFICATION_NOT_RECEIVED))
//					||(domain.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_CLARIFICATION_NEEDED_FROM_MEMBER)
//						&& domain.getRecommendationStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_CLARIFICATION_RECEIVED))) {
//				
...
//			}
			...
			if(isMinistryChanged || isSubDepartmentChanged){
				Status sendToDepartmentStatus = Status.findByType(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_SENDTODEPARTMENT, locale.toString());
				if(workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.DEPARTMENT)
						|| workflowDetails.getAssigneeUserGroupType().equals(ApplicationConstants.DEPARTMENT_DESKOFFICER)){
...
//				if(clubbedEntities!=null) {
//					for(ClubbedEntity ce: clubbedEntities) {
//						AppropriationBillMotion clubbedAppropriationBillMotion = ce.getAppropriationBillMotion();
//						clubbedAppropriationBillMotion.setMinistry(domain.getMinistry());
//						clubbedAppropriationBillMotion.setSubDepartment(domain.getSubDepartment());
//						clubbedAppropriationBillMotion.merge();
//					}
//				}			
				domain.merge();
				appropriationBillMotion = AppropriationBillMotion.findById(AppropriationBillMotion.class,domain.getId()); //added in order to avoid optimistic lock exception
				...
				Workflow workflow = Workflow.findByStatus(appropriationBillMotion.getInternalStatus(), locale.toString());
				//Motion in Post final status and pre ballot state can be group changed by Department 
				//as well as assistant of Secretariat
				WorkflowDetails.startProcessAtGivenLevel(appropriationBillMotion, ApplicationConstants.APPROVAL_WORKFLOW, workflow, ugt, assigneeLevel, locale.toString());
		...
		return "workflow/appropriationbillmotion/" + userGroupType;
	}

	private void performAction(final AppropriationBillMotion domain) {
		try{
			String internalStatus=domain.getInternalStatus().getType();
			String recommendationStatus=domain.getRecommendationStatus().getType();
			if(internalStatus.equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION)
					&&recommendationStatus.equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION)){
				performActionOnAdmission(domain);
			}
			else if(internalStatus.equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_REJECTION)
					&&recommendationStatus.equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_REJECTION)){
	...
	private void performActionOnAdmission(final AppropriationBillMotion domain) {
		Status finalStatus=Status.findByType(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION, domain.getLocale());
...
//			}
//			
//			Status status=Status.findByType(ApplicationConstants.APPROPRIATIONBILLMOTION_PUTUP_NAME_CLUBBING, domain.getLocale());
//			for(ClubbedEntity i:clubbedEntities){
//				AppropriationBillMotion motion=i.getAppropriationBillMotion();
//				if(motion.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_SYSTEM_CLUBBED)){
...
//		}
	...
	private void performActionOnRejection(AppropriationBillMotion domain) throws ELSException {
...
//		AppropriationBillMotion.updateClubbing(domain);
		
	}
	
	private void performActionOnClarificationNeededFromDepartment(AppropriationBillMotion domain) {
	...
	private void performActionOnClarificationNeededFromMember(AppropriationBillMotion domain) {
		if(domain.getRevisedMainTitle() == null || domain.getRevisedMainTitle().isEmpty()){			
			domain.setRevisedMainTitle(domain.getMainTitle());			
		}
	...
	private void performActionOnClarificationNeededFromMemberAndDepartment(AppropriationBillMotion domain) {
		if(domain.getRevisedMainTitle() == null || domain.getRevisedMainTitle().isEmpty()){			
			domain.setRevisedMainTitle(domain.getMainTitle());			
		}
	...
	private void performActionOnClarificationReceived(AppropriationBillMotion domain) {
	...
	private void performActionOnClubbing(AppropriationBillMotion domain) throws ELSException {
...
//		AppropriationBillMotion.updateClubbing(domain);
	...
	private void performActionOnClubbingRejection(AppropriationBillMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
////		AppropriationBillMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnNameClubbing(AppropriationBillMotion domain) throws ELSException {
...
//		AppropriationBillMotion.updateClubbing(domain);
	...
	private void performActionOnNameClubbingRejection(AppropriationBillMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
//		AppropriationBillMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnClubbingPostAdmission(AppropriationBillMotion domain) throws ELSException {
...
//		AppropriationBillMotion.updateClubbing(domain);
	...
	private void performActionOnClubbingRejectionPostAdmission(AppropriationBillMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
//		AppropriationBillMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnUnclubbing(AppropriationBillMotion domain) throws ELSException {
		/**** remove clubbing (status is changed accordingly in unclub method itself) ****/
//		AppropriationBillMotion.unclub(domain, domain.getLocale());
	...
	private void performActionOnUnclubbingRejection(AppropriationBillMotion domain) throws ELSException {
...
//		domain.setRecommendationStatus(domain.getParent().getInternalStatus());
	...
	private void performActionOnClarificationNotReceived(AppropriationBillMotion domain) {
		Status newStatus = Status.findByType(ApplicationConstants.APPROPRIATIONBILLMOTION_SYSTEM_ASSISTANT_PROCESSED, domain.getLocale());
		domain.setInternalStatus(newStatus);
		domain.setActor(null);
		domain.setLocalizedActorName("");
		domain.setWorkflowDetailsId(null);
		...
		try {
			...
			if (strHouseType != null && !(strHouseType.isEmpty())
				...
				if (strWorkflowSubType.equals("request_to_supporting_member")) {
					...
					CustomParameter deviceTypeInternalStatusUsergroup = CustomParameter.findByName(CustomParameter.class, "APPROPRIATIONBILLMOTION_PUT_UP_OPTIONS_" + motionType.getType().toUpperCase() + "_" + internalStatus.getType().toUpperCase() + "_" + strUserGroupType.toUpperCase(), "");
					CustomParameter deviceTypeHouseTypeUsergroup = CustomParameter.findByName(CustomParameter.class, "APPROPRIATIONBILLMOTION_PUT_UP_OPTIONS_" + motionType.getType().toUpperCase() + "_" + houseType.getType().toUpperCase() + "_" + strUserGroupType.toUpperCase(), "");
					CustomParameter deviceTypeUsergroup = CustomParameter.findByName(CustomParameter.class, "APPROPRIATIONBILLMOTION_PUT_UP_OPTIONS_" + motionType.getType() .toUpperCase() + "_" + strUserGroupType.toUpperCase(), "");
					if (finalApprovingAuthority != null
							&& finalApprovingAuthority.getValue().contains(strUserGroupType)) {
						CustomParameter finalApprovingAuthorityStatus = CustomParameter.findByName(CustomParameter.class, "APPROPRIATIONBILLMOTION_PUT_UP_OPTIONS_" + strUserGroupType.toUpperCase(), "");
						if (finalApprovingAuthorityStatus != null) {
			...
			return "workflow/appropriationbillmotion/bulkapprovalinit";
	...
	public String getBulkApprovalView(final HttpServletRequest request,
		...
		return "workflow/appropriationbillmotion/bulkapprovalview";
	}
			...
			for (String i : selectedItems) {
				try {
					if (strWorkflowSubType.equals("request_to_supporting_member")) {
						...
						WorkflowDetails wfDetails = WorkflowDetails.findById(WorkflowDetails.class, id);
						AppropriationBillMotion motion = AppropriationBillMotion.findById(AppropriationBillMotion.class, Long.parseLong(wfDetails.getDeviceId()));
...
//							}
							...
							if (reference != null && reference.getId() != null
								...
								if(status != null){
									if(!status.getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_RECOMMEND_DISCUSS) 
									&& !status.getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_RECOMMEND_SENDBACK)
									&& !status.getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_SEND_TO_DEPARTMENT)
									&& !status.getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_PROCESSED_SEND_TO_SECTIONOFFICER)){
										motion.setInternalStatus(status);
									}
									motion.setRecommendationStatus(status);	
...
//									}
								...
								} else if (motion.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_ADMISSION)) {
									admittedMsg.append(motion.formatNumber() + ",");
								} else if (motion.getInternalStatus().getType().equals(ApplicationConstants.APPROPRIATIONBILLMOTION_FINAL_REJECTION)) {
	...
	private void populateBulkApprovalView(final Model model,
		...
		if (strHouseType != null && !(strHouseType.isEmpty())
			...
			for (WorkflowDetails i : workflowDetails) {
				...
				AppropriationBillMotion motion = AppropriationBillMotion.findById(AppropriationBillMotion.class, Long.parseLong(i.getDeviceId()));
...
//	private void findLatestRemarksByUserGroup(final AppropriationBillMotion domain, final ModelMap model,
//			final HttpServletRequest request)throws ELSException {
//		Map<String, String[]> requestMap=new HashMap<String, String[]>();			
//		requestMap.put("appropriationBillMotionId",new String[]{String.valueOf(domain.getId())});
