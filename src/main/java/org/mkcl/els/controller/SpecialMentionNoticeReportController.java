...
@Controller
@RequestMapping("specialmentionnotice/report")
public class SpecialMentionNoticeReportController extends BaseController{
	...
	public String getReport(HttpServletRequest request, Model model, Locale locale){
		...
		return "specialmentionnotice/reports/"+request.getParameter("reportout");		
	}
	...
	public String getCurrentStatusReport(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale){
		...
		return "specialmentionnotice/reports/statusreport";
	}
	...
	public String getCurrentStatusReportVM(@PathVariable("moId") Long id, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale){
		...
		response.setContentType("text/html; charset=utf-8");		
		return SpecialMentionNoticeReportHelper.getCurrentStatusReportData(id, model, request, response, locale);
	}
	...
	public @ResponseBody void generateBhag1Report(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
		...
		if(specialMentionNoticeDateStr==null || specialMentionNoticeDateStr.isEmpty() 
			...
			isError = true;
			errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.bhag1.parameterNotSet", locale.toString());						
		} else {
			...
			CustomParameter bhag1StatusAllowedCP = CustomParameter.findByName(CustomParameter.class, "SMIS_BHAG_1_ALLOWED_STATUS_TYPES", "");
			...
			if(bhag1StatusAllowedCP!=null && bhag1StatusAllowedCP.getValue()!=null && !bhag1StatusAllowedCP.getValue().isEmpty()) {
				...
				allowedStatusTypesForBhag1 = ApplicationConstants.SPECIALMENTIONNOTICE_RECOMMEND_ADMISSION + "," + ApplicationConstants.SPECIALMENTIONNOTICE_FINAL_ADMISSION;
			}
			...
			try {
				reportFile = generateReportUsingFOP(new Object[] {bhag1Motions}, "smis_bhag1_template", "WORD", "smis_bhag1", locale.toString());
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("**** Some error occurred ****");
				isError = true;
				errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.bhag1.someErrorOccurred", locale.toString());
			}
			System.out.println("SMIS Bhag 1 Report generated successfully in WORD format!");
	...
	public @ResponseBody void generateBhag2Report(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
		...
		if(specialMentionNoticeDateStr==null || specialMentionNoticeDateStr.isEmpty() 
			...
			logger.error("**** One of the request parameters is not set ****");
			isError = true;
			errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.bhag2.parameterNotSet", locale.toString());						
		} else {
			Date specialMentionNoticeDate = FormaterUtil.formatStringToDate(specialMentionNoticeDateStr, ApplicationConstants.SERVER_DATEFORMAT, locale.toString());
			requestMap.put("specialMentionNoticeDate", new String[] {FormaterUtil.formatDateToString(specialMentionNoticeDate, ApplicationConstants.DB_DATEFORMAT)});
			requestMap.put("sessionId", new String[] {sessionId});
			CustomParameter bhag2StatusAllowedCP = CustomParameter.findByName(CustomParameter.class, "SMIS_BHAG_2_ALLOWED_STATUS_TYPES", "");
			...
			if(bhag2StatusAllowedCP!=null && bhag2StatusAllowedCP.getValue()!=null && !bhag2StatusAllowedCP.getValue().isEmpty()) {
				...
				allowedStatusTypesForBhag2 = ApplicationConstants.SPECIALMENTIONNOTICE_RECOMMEND_REJECTION + "," + ApplicationConstants.SPECIALMENTIONNOTICE_FINAL_REJECTION;
			}
			...
			try {
				reportFile = generateReportUsingFOP(new Object[] {bhag2Motions}, "smis_bhag2_template", "WORD", "smis_bhag2", locale.toString());
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("**** Some error occurred ****");
				isError = true;
				errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.bhag2.someErrorOccurred", locale.toString());
			}
			System.out.println("SMIS Bhag 2 Report generated successfully in WORD format!");
	...
	public @ResponseBody void generateStatementReport(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
		...
		if(specialMentionNoticeDateStr==null || specialMentionNoticeDateStr.isEmpty() 
			...
			logger.error("**** One of the request parameters is not set ****");
			isError = true;
			errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.statement.parameterNotSet", locale.toString());						
		} else {
			Date specialMentionNoticeDate = FormaterUtil.formatStringToDate(specialMentionNoticeDateStr, ApplicationConstants.SERVER_DATEFORMAT, locale.toString());
			requestMap.put("specialMentionNoticeDate", new String[] {FormaterUtil.formatDateToString(specialMentionNoticeDate, ApplicationConstants.DB_DATEFORMAT)});
			requestMap.put("sessionId", new String[] {sessionId});
			...
			requestMap.put("locale", new String[]{locale.toString()});
			@SuppressWarnings("rawtypes")
			List bhag1Motions = Query.findReport("SMIS_BHAG1_REPORT", requestMap, true);
			/**** Bhag 2 list ****/
			CustomParameter bhag2StatusAllowedCP = CustomParameter.findByName(CustomParameter.class, "SMIS_BHAG_2_ALLOWED_STATUS_TYPES", "");
			String allowedStatusTypesForBhag2 = "";
			if(bhag2StatusAllowedCP!=null && bhag2StatusAllowedCP.getValue()!=null && !bhag2StatusAllowedCP.getValue().isEmpty()) {
			...
			requestMap.put("locale", new String[]{locale.toString()});
			@SuppressWarnings("rawtypes")
			List bhag2Motions = Query.findReport("SMIS_BHAG2_REPORT", requestMap, true);
			try {
				reportFile = generateReportUsingFOP(new Object[] {bhag1Motions, bhag2Motions}, "smis_statement_template", "WORD", "smis_bhag2", locale.toString());
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("**** Some error occurred ****");
				isError = true;
				errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.statement.someErrorOccurred", locale.toString());
			}
			System.out.println("SMIS Bhag 2 Report generated successfully in WORD format!");
	...
	public @ResponseBody void generateSubmittedMotionsReport(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
		...
		if(sessionId==null || sessionId.isEmpty() || specialMentionNoticeDateStr==null || specialMentionNoticeDateStr.isEmpty() || reportQueryName==null || reportQueryName.isEmpty()) {
			...
			try {
				reportFile = generateReportUsingFOP(new Object[] {submittedMotions}, "smis_submitted_motions_template", "WORD", "smis_submitted_motions", locale.toString());
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("**** Some error occurred ****");
				isError = true;
				errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.submitted_motions_report.someErrorOccurred", locale.toString());
			}
	...
	public @ResponseBody void generateAdmittedMotionsReport(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
		...
		if(sessionId==null || sessionId.isEmpty() || specialMentionNoticeDateStr==null || specialMentionNoticeDateStr.isEmpty() || reportQueryName==null || reportQueryName.isEmpty()) {
			...
			try {
				reportFile = generateReportUsingFOP(new Object[] {admittedMotions}, "smis_admitted_motions_template", "WORD", "smis_admitted_motions", locale.toString());
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("**** Some error occurred ****");
				isError = true;
				errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.admitted_motions_report.someErrorOccurred", locale.toString());
			}
			System.out.println("SMIS Admitted Motions Report generated successfully in WORD format!");
	...
	public @ResponseBody void generateRejectedMotionsReport(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
		...
		if(sessionId==null || sessionId.isEmpty() || specialMentionNoticeDateStr==null || specialMentionNoticeDateStr.isEmpty() || reportQueryName==null || reportQueryName.isEmpty()) {
			...
			try {
				reportFile = generateReportUsingFOP(new Object[] {rejectedMotions}, "smis_rejected_motions_template", "WORD", "smis_rejected_motions", locale.toString());
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("**** Some error occurred ****");
				isError = true;
				errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.rejected_motions_report.someErrorOccurred", locale.toString());
			}
			System.out.println("SMIS Rejected Motions Report generated successfully in WORD format!");
	...
	public @ResponseBody void generateRegisterReport(final HttpServletRequest request, HttpServletResponse response, final Locale locale, final ModelMap model){
		...
		if(sessionId==null || sessionId.isEmpty() || reportQueryName==null || reportQueryName.isEmpty()) {
			...
			try {
				reportFile = generateReportUsingFOP(new Object[] {registerMotions}, "smis_register_template", "WORD", "smis_register", locale.toString());
			} catch (Exception e) {
				e.printStackTrace();
				logger.error("**** Some error occurred ****");
				isError = true;
				errorMessage = MessageResource.findByFieldName(MessageResource.class, "code", "smis.register_report.someErrorOccurred", locale.toString());
			}
			System.out.println("SMIS Register Report generated successfully in WORD format!");
	...
	private void generateIntimationLetter(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale){
		...
		try{
			...
			if(spmNumber != null && !spmNumber.isEmpty()){
				...
				String templateName = "specialmentionnotice_intimation_letter_template";
...
class SpecialMentionNoticeReportHelper{
	private static Logger logger = LoggerFactory.getLogger(SpecialMentionNoticeReportHelper.class);
	...
	public static String getCurrentStatusReportData(Long id, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale){
		String page = "specialmentionnotice/error";
		try{
			String strDevice = request.getParameter("device"); 
			if(strDevice != null && !strDevice.isEmpty()){
				SpecialMentionNotice qt = SpecialMentionNotice.findById(SpecialMentionNotice.class, id);
				List report = generatetCurrentStatusReport(qt, strDevice, locale.toString());				
				Map<String, Object[]> dataMap = new LinkedHashMap<String, Object[]>();
				if(report != null && !report.isEmpty()){
					
					List<User> users = User.findByRole(false, "SMIS_PRINCIPAL_SECRETARY", locale.toString());
					model.addAttribute("principalSec", users.get(0).getTitle() + " " + users.get(0).getFirstName() + " " + users.get(0).getLastName());
	
					CustomParameter csptAllwedUserGroupForStatusReportSign = CustomParameter.findByName(CustomParameter.class, (qt.getHouseType().getType().equals(ApplicationConstants.LOWER_HOUSE)? "SMIS_ALLOWED_USERGROUPS_FOR_STATUS_REPORT_SIGN_LOWERHOUSE": "SMIS_ALLOWED_USERGROUPS_FOR_STATUS_REPORT_SIGN_UPPERHOUSE"), "");
					if(csptAllwedUserGroupForStatusReportSign != null){
						if(csptAllwedUserGroupForStatusReportSign.getValue() != null && !csptAllwedUserGroupForStatusReportSign.getValue().isEmpty()){
							...
							if(csptCurrentStatusAllowedBeforeApproval!=null && csptCurrentStatusAllowedBeforeApproval.getValue()!=null
								...
								wfConfig = WorkflowConfig.getLatest(qt, ApplicationConstants.SPECIALMENTIONNOTICE_RECOMMEND_ADMISSION, locale.toString());
							...
							CustomParameter onPaperSigningAuthorityParameter = CustomParameter.findByName(CustomParameter.class, "SMIS_CURRENTSTATUS_ONPAPER_SIGNING_AUTHORITY_"+qt.getHouseType().getType(), "");
								...
								for(String str : signingAuthorities){
									...
									if(dataMap.get(str) == null){
										...
										if(authority.equals(ApplicationConstants.UNDER_SECRETARY_COMMITTEE)){
											 ...
											 ref = UserGroup.findSpecialMentionNoticeActor(qt, authority, String.valueOf(0), locale.toString());
										}else{
											 userGroupType = UserGroupType.
													findByFieldName(UserGroupType.class, "type", str, locale.toString());
											 ref = UserGroup.findSpecialMentionNoticeActor(qt, str, String.valueOf(0), locale.toString());
										}
					...
					page = (qt.getHouseType().getType().equals(ApplicationConstants.LOWER_HOUSE))? "specialmentionnotice/reports/statusreportlowerhouse": "specialmentionnotice/reports/statusreportupperhouse";
				}		
	...
	@SuppressWarnings("rawtypes")
	public static List generatetCurrentStatusReport(final SpecialMentionNotice motion, final String device, final String locale){
		...
		List list = Query.findReport("SMIS_CURRENTSTATUS_REPORT", parameters);
		for(Object o : list){
