...
public class MenuItemController extends GenericController<MenuItem> {
    ...
    protected void populateNew(final ModelMap model,
                               final MenuItem domain,
                               final Locale locale,
                               final HttpServletRequest request) {
        ((BaseDomain) domain).setLocale(locale.toString());
        String parentId = request.getParameter("parentId");
        if (parentId != null) {
            MenuItem parent = MenuItem.findById(
                    MenuItem.class, Long.parseLong(parentId));
    ...
    private void customValidate(final MenuItem domain,
        ...
        if (duplicateParameter) {
            result.rejectValue(
                    "textKey", "NonUnique", params, "Duplicate Parameter");
        }
