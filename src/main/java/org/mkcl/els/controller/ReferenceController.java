...
//		List<MemberMinister> ministries=selectedGroup.getMinistries();
//		List<MasterVO> ministriesVO=new ArrayList<MasterVO>();
//		List<MasterVO> departmentsVO=new ArrayList<MasterVO>();
//		int count=0;
//		for(MemberMinister i:ministries){
//			MasterVO masterVOMinister=new MasterVO(i.getId(),i.getMinistry().getName());
//			if(count==0){
//				List<MemberDepartment> memberDepartments=i.getMemberDepartments();
//				for(MemberDepartment j:memberDepartments){
//					MasterVO masterVODepartment=new MasterVO(j.getId(),j.getDepartment().getName());
//					departmentsVO.add(masterVODepartment);
//				}
//			}
//			ministriesVO.add(masterVOMinister);
//		}
		//populating answering dates
		List<QuestionDates> dates=selectedGroup.getQuestionDates();
		List<MasterVO> answeringDates=new ArrayList<MasterVO>();
		for(QuestionDates i:dates){
			...
			if(ansDate!=null){
				...
				MasterVO answeringDateVO=new MasterVO(i.getId(),FormaterUtil.formatMonthsMarathi(strAnsweringDate, locale.toString()));
				answeringDates.add(answeringDateVO);
			}
		}
		//groupVO.setDepartments(departmentsVO);
		//groupVO.setMinistries(ministriesVO);
