...
;(function ($) {
    $.widget("ui.sexyselect", {
         el : { s:'<span />', d:'<div />', li : '<li />', i : '<img />', u: '<ul />' , a : '<a />', p :'<input />', o: '<option />' }, //element definitions. do not change
         revert : false,
         ...
         options: { 
            ...
            selectionMode : 'none', //type of selection required : this function will be overriden if the select has the attr multiple added to it.
                                   ...
                                   //                             multiple = checkbox
            defaultCheckAllText: 'Check All',
            defaultUnCheckAllText: 'Uncheck All'
        },
        _init: function(){
			...
			if(options.selectionMode == 'multiple') {				
				this.checkAllDiv = $(self.el.d)
                			.css({'border-bottom' : 'solid 1px #f1f1f1', 'font-size' : '0.8em' , 'color' : '#666', 'height' : '23px', 'line-height' : '23px'}),
                checkAll = $(self.el.a)
                			.attr('href','#')
                			.attr('id',initElem.attr('id') + '_check_all')
                			.text(options.defaultCheckAllText)
                			.addClass('ui-widget-content')
                			.css({'color' : '#999', 
                				'width' : options.width - 110 + 'px' ,
                				'text-decoration' : 'none',
                				'text-align' : 'center',
                				'text-color' : '#2779AA !important',             				
                				'font-style' : 'italic',
                				'font-weight' : 'normal',
                ...
                uncheckAll = $(self.el.a)
                			.attr('href','#')
                			.attr('id',initElem.attr('id') + '_uncheck_all')
                			.text(options.defaultUnCheckAllText)
                			.addClass('ui-widget-content')
                			.css({'color' : '#999', 
                				'width' : options.width - 110 + 'px' ,
                				'text-decoration' : 'none',
				...
				this.checkAllDiv.append(checkAll);
                this.checkAllDiv.append('&nbsp;&nbsp;&nbsp;&nbsp;');	
                this.checkAllDiv.append(uncheckAll);                
				this.sselect.append(this.checkAllDiv);
			}
        ...
        _syncItems : function()
        {
            ...
            initElem.children().each(function(){
                    var opt = $(this),
                        ...
                        item = self._createItem(text,value,alt, checked);
