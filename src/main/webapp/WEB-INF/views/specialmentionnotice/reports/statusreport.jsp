...
<head>
	<title>
	<spring:message code="specialmentionnotice.statusreport" text="Status Report"/>
	</title>
	...
	<script type="text/javascript">
		...
		function showRemarkReport(){
			...
			'&grid='+ strLocation + 
			'&specialmentionNotieDate='+((selectedSpecialMentionNoticeDate=='' || selectedSpecialMentionNoticeDate==undefined)?'0':selectedSpecialMentionNoticeDate);
			
			$.ajax({
				url: 'ref/pendingtasksdevicessmis'+paramVar,
		         async:   false,
		...
		function addRemarkReport(){
			if(ids.length > 0 && counter < ids.length){
				 $.get('specialmentionnotice/report/'+ ids[counter] + '/currentstatusreportvm?device='+$("#device").val(),function(data1){
