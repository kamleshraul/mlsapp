...
<head>
	...
	function loadGroups(housetype,year,sessiontype,locale){
		$.get('ref/groups?housetype='+housetype+'&year='+year+'&sessiontype='+sessiontype+'&locale='+locale,function(data){
			...
			if(data.length>0){
				...
				loadDepartments(data[i].name,housetype,year,sessiontype,locale);
			}
		});		
	}

	function loadDepartments(group,housetype,year,sessiontype,locale){
		$.get('ref/departments?group='+group+'&housetype='+housetype+'&year='+year+'&sessiontype='+sessiontype+'&locale='+locale,function(data){
			...
			if(data.length>0){
				...
				loadSubDepartments(group,housetype,year,sessiontype,locale);
			}
		});	
	}

	function loadSubDepartments(group,housetype,year,sessiontype,locale){
		$.get('ref/subdepartments?group='+group+'&housetype='+housetype+'&year='+year+'&sessiontype='+sessiontype+'&locale='+locale,function(data){
	...
	$('document').ready(function(){	
		...
		loadGroups($("#param_HOUSETYPE_"+locale).val(),$("#param_YEAR_"+locale).val(),$("#param_SESSIONTYPE_"+locale).val(),locale);
		$.unblockUI();	
		});
		...
		$.blockUI({ message: '<img src="./resources/images/waitAnimated.gif" />' }); 		
		loadDepartments($("#param_GROUP_"+locale).val(),$("#param_HOUSETYPE_"+locale).val(),$("#param_HOUSETYPE_"+locale).val(),$("#param_YEAR_"+locale).val(),$("#param_SESSIONTYPE_"+locale).val(),locale);
		$.unblockUI();
		});	

		$("#param_DEPARTMENT_"+locale).change(function(){
		$.blockUI({ message: '<img src="./resources/images/waitAnimated.gif" />' }); 			
		loadSubDepartments($("#param_GROUP_"+locale).val(),$("#param_HOUSETYPE_"+locale).val(),$("#param_HOUSETYPE_"+locale).val(),$("#param_YEAR_"+locale).val(),$("#param_SESSIONTYPE_"+locale).val(),locale);
		$.unblockUI();	
		});	
	});		
</script>
...
<form:form action="usergroup" method="POST"  modelAttribute="domain">
	...
	<p>
		...
		<select  id="param_HOUSETYPE_${locale}" name="param_HOUSETYPE_${locale}" multiple="multiple" size="2">
	...
	<p>
		...
		<select  id="param_GROUP_${locale}" name="param_GROUP_${locale}" multiple="multiple" size="5">
			...
			<c:choose>
			<c:when test="${selectedgroup==i.number }">
