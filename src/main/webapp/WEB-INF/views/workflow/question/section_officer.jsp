...
<head>
	...
	$(document).ready(function(){
	    ...
	    $("#sendback").click(function(e){
			...
			$("#sendbackactor").val($("#backactor").val());
			$("#flag").val("question_after_approval_sendback_to_assistant_for_correction");
			console.log($("#sendbackactor").val());
	    ...
	    /**** Answer received ****/
	    $("#markasanswered").click(function(e){
			//removing <p><br></p>  from wysiwyg editor
			$(".wysiwyg").each(function(){
				var wysiwygVal=$(this).val().trim();
				if(wysiwygVal=="<p></p>"||wysiwygVal=="<p><br></p>"||wysiwygVal=="<br><p></p>"){
			...
			$("#flag").val("question_after_approval_answer_received_from_department");
			$("#endflag").val("end");					
			$.prompt($('#startWorkflowMessage').val(),{
				buttons: {Ok:true, Cancel:false}, callback: function(v){
		        if(v){
...
<form:form action="workflow/question/mytask" method="PUT" modelAttribute="domain">
	...
	<c:if test="${newRecommendationStatus=='question_after_approval_send_to_department' }">
	<p id="actorDiv" >
	...
	<c:if test="${newRecommendationStatus=='question_after_approval_put_for_dateapproval' }">
	<p id="actorDiv" >
	<label class="small"><spring:message code="question.nextactor" text="Next Users"/></label>
	<select id="actor" name="actor" class="sSelect">
	...
	<div class="fields">
		...
		<input id="dateapproval" type="button" value="<spring:message code='generic.dateapproval' text='Put Up For Date Approval'/>" class="butDef">
		</c:when>
		<c:when test="${newRecommendationStatus!='question_after_approval_department_sent_answer' }">
		<input id="startworkflow" type="button" value="<spring:message code='generic.submit' text='Submit'/>" class="butDef">
		<input id="sendback" type="button" value="<spring:message code='generic.sendback' text='Send Back To Assistant'/>" class="butDef">
		</c:when>		
		<c:otherwise>
		<input id="markasanswered" type="button" value="<spring:message code='generic.markasanswered' text='Answer Received'/>" class="butDef">
		</c:otherwise>
		</c:choose>		
		</p>
	</div>
...
<p id="sendbackactorDiv" style="display:none;">
	<label class="small"><spring:message code="question.nextactor" text="Next Users"/></label>
	<select id="backactor" name="backactor" class="sSelect">
	<c:forEach items="${sendbackactors}" var="i">
	<option value="${i.id}">${i.name }</option>
	</c:forEach>	
	</select>
</p>
