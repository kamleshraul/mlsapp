...
<head>
	...
	function loadActors(value){
		if(value!='-'){
			...
			if(deviceTypeType == 'questions_starred') {
				...
				dateAdmitted = $("#internalStatusMaster option[value='question_processed_final_dateAdmitted']").text();
				dateResubmit = $("#internalStatusMaster option[value='question_processed_final_dateResubmit']").text();
				...
				dateAdmitted = $("#internalStatusMaster option[value='question_unstarred_processed_final_dateAdmitted']").text();
				dateResubmit = $("#internalStatusMaster option[value='question_unstarred_processed_final_dateResubmit']").text();
				...
				dateAdmitted = $("#internalStatusMaster option[value='question_halfHourFromQuestion_processed_final_dateAdmitted']").text();
				dateResubmit = $("#internalStatusMaster option[value='question_halfHourFromQuestion_processed_final_dateResubmit']").text();
	...
	$(document).ready(function(){
		...
		$("#submitBulkEdit").click(function(e){
			...
			$(".wysiwyg").each(function(){
				var wysiwygVal=$(this).val().trim();
				if(wysiwygVal == "<p></p>" || wysiwygVal == "<p><br></p>" 
						|| wysiwygVal  == "<br><p></p>"){
					$(this).val("");
				}
			});								
...
<form:form action="workflow/question" method="PUT" modelAttribute="domain">
	...
	<p>
		<c:if test="${selectedQuestionType!='questions_halfhourdiscussion_from_question'}">
			...
			<a href="#" id="reviseQuestionText" style="margin-left: 20px;"><spring:message code="question.reviseQuestionText" text="Revise Question text"></spring:message></a>
		</c:if>
