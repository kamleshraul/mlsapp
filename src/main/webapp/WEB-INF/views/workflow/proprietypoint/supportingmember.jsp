...
<head>
	...
	$(document).ready(function(){
	    ...
	    $("#viewStatus").click(function(){
		    $.get('proprietypoint/status/'+$("#proprietyPoint").val(),function(data){
			    $.fancybox.open(data);
		    }).fail(function(){
				$.unblockUI();
				if($("#ErrorMsg").val()!=''){
...
<form:form action="workflow/proprietypoint/supportingmember" method="PUT" modelAttribute="domain">
	...
	<p>
		<label class="wysiwyglabel"><spring:message code="proprietypoint.pointsOfPropriety" text="Details"/>*</label>
		<form:textarea path="approvedText" cssClass="wysiwyg" readonly="true"></form:textarea>
	</p>	
	<c:choose>
	<c:when test="${status=='COMPLETED'}">
	<p>
	<label class="small"><spring:message code="proprietypoint.decisionstatus" text="Decision?"/>*</label>
	...
	<input type="hidden" id="proprietyPoint" name="proprietyPoint" value="${proprietyPoint}">
</form:form>
</div>
