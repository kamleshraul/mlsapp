	...
	$(document).ready(function(){
		...
		$('#changeRuleSuspensionDate').click(function() {
			...
			$.prompt('Do you really want to change the adjourning date?', {
				...
				callback: function(v) {
					if(v) {
						$('#rulesSuspensionDate').removeAttr('disabled');
	...
	<div id="assistantDiv">
		<form:form action="workflow/rulessuspensionmotion" method="PUT" modelAttribute="domain">
			...
			<p>
				...
				<label class="small"><spring:message code="rulessuspensionmotion.rulesuspensiondate" text="Rule Suspension Date"/></label>
				...
				<input id="rulesSuspensionDate" name="rulesSuspensionDate" type="hidden"  value="${selectedRuleSuspensionDate}">
			</p>		
			...
			<c:choose>
			<c:when test="${internalStatusType == 'rulessuspensionmotion_final_admission'}">
			<p>
			...
			<div class="fields">
				...
				<%-- <c:if test="${currTimeMillis <= sendbacktimelimit}"> --%>
					<input id="sendBack" type="button" value="<spring:message code='rulessuspensionmotion.sendback' text='Send Back'/>" class="butDef">
				<%-- </c:if> --%>
