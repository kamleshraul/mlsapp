	...
	$(document).ready(function(){
	 ...
	 $("#actorUpperHouse").change(function(){
	    ...
	    $('#submit').click(function(e) {
			...
			var deviceTypeSelected = $('#selectedDeviceType').val();
			if(deviceTypeSelected == 'resolutions_government') {
				...
				$.ajax({
					url: "ref/resolutions_government/isDiscussionDateChanged?expectedDiscussionDate="+expectedDiscussionDate+"&requestedDiscussionDate="+requestedDiscussionDate,
					async: false,
					success: function(data) {											
						if(data == "true") {							
							if($('#remarks').val() == "") {
				    			$.prompt("Please enter remarks for changing discussion date...");
				    			e.preventDefault();
				    		}		    		
						} else if(data == "" || data == undefined) {
							$.prompt("error in discussion date...");	
							e.preventDefault();
						}					
					}
				}).fail(function(){
...
<form:form action="workflow/resolution" method="PUT" modelAttribute="domain">
	...
	<c:if test="${selectedDeviceType == 'resolutions_government'}">
		...
		<form:input id="discussionDate" path="discussionDate" cssClass="datemask sText" readonly="${isDiscussionDateReadOnly}"/>
		<input type="hidden" id="expectedDiscussionDate" value="${expectedDiscussionDate}"/>
