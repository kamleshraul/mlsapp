...
<head>
	...
	$(document).ready(function(){
	    ...
	    $("#viewStatus").click(function(){
		    $.get('adjournmentmotion/status/'+$("#adjournmentMotion").val(),function(data){
			    $.fancybox.open(data);
		    }).fail(function(){
				$.unblockUI();
				if($("#ErrorMsg").val()!=''){
...
<form:form action="workflow/adjournmentmotion/supportingmember" method="PUT" modelAttribute="domain">
	...
	<p>
		...
		<a href="#" id="viewStatus"><spring:message code="adjournmentmotion.viewstatus" text="View Status"></spring:message></a>		
	</p>
	...
	<p>
		<label class="wysiwyglabel"><spring:message code="adjournmentmotion.details" text="Details"/>*</label>
		<form:textarea path="approvedText" cssClass="wysiwyg" readonly="true"></form:textarea>
	</p>	
	<c:choose>
	<c:when test="${status=='COMPLETED'}">
	<p>
	<label class="small"><spring:message code="adjournmentmotion.decisionstatus" text="Decision?"/>*</label>
	...
	<input type="hidden" id="adjournmentMotion" name="adjournmentMotion" value="${adjournmentMotion}">
</form:form>
</div>
