...
<head>
	...
	function refreshEdit(id){
		...
		//$("#assistantDiv").show();
		//scrollTop();
		showTabByIdAndUrl('process_tab', 'workflow/myTasks/' + $("#wfId").val() + '/process'+'?action=edited');
		$.unblockUI();			
	}
	/**** load actors ****/
	function loadActors(value){
		...
		if(value!='-'){
			...
			var putUpForDateApproval=$("#internalStatusMaster option[value='standalonemotion_processed_putUpForDateApproval']").text();
			var sendToSectionOfficer = $("#internalStatusMaster option[value='standalonemotion_processed_sendToSectionOfficer']").text();
			var revisedDraftRecommendApproval = $("#internalStatusMaster option[value='standalonemotion_recommend_revisedtextapproval']").text();
	...
	function loadGroup(ministry){
		...
		$.get('ref/ministry/'+ministry+'/group?houseType='+$("#houseType").val()+'&sessionYear='+$("#sessionYear").val()+'&sessionType='+$("#sessionType").val(),function(data){
			...
			$("#group").val(data.id);
			loadSubDepartments(ministry,null);			
		}).fail(function(){
			if($("#ErrorMsg").val()!=''){
				$("#error_p").html($("#ErrorMsg").val()).css({'color':'red', 'display':'block'});
			}else{
...
<form:form action="workflow/standalonemotion" method="PUT" modelAttribute="domain">
	...
	<p>
		...
		<c:choose>
			<c:when test="${domain.number!=null}">
	...
	<c:if test="${selectedQuestionType=='motions_standalonemotion_halfhourdiscussion' and houseTypeType=='lowerhouse1'}">
		<p>
	...
	<p>
		...
		<c:forEach items="${internalStatusMaster}" var="i">
...
<input id="currentUserLevel" value="${currentUserLevel}" type="hidden" />
...
<input type="hidden" id="wfId" value="${workflowdetails}" />
