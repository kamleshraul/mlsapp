...
<head>
	...
	/* =============== ACTIONS =============== */
	function showCommitteeSubjectList() {
		var id = 'list_tab';
		var url = 'committeesubject/list' + '?' + getGridURLParams();
		showTabByIdAndUrl(id, url);
	}
	
	function newCommitteeSubject() {
		$('#cancelFn').val('newCommitteeSubject');
		$('#key').val('');
		var id = 'details_tab';
		var url = 'committeesubject/new'+'?'+getGridURLParams();
		showTabByIdAndUrl(id, url);
	}

	function editCommitteeSubject(rowId) {
		$('#cancelFn').val('editCommitteeSubject');
		if(rowId == null || rowId == '') {
		...
		var id = 'details_tab';
		var url = 'committeesubject' + '/' + rowId + '/' + 'edit?' + getGridURLParams();
		showTabByIdAndUrl(id, url);
	}

	function deleteCommitteeSubject(rowId) {
		if(rowId == null || rowId == '') {
			var msg = $('#selectRowFirstMessage').val();
			$.prompt(msg);
			return;
		...
		var options = {buttons: {Ok:true, Cancel:false}, 
					   callback: function(v){
			   				if(v){
								$.delete_('committeesubject' + '/' + rowId + '/' + 'delete', 
										null, 
	...
	function searchCommitteeSubject() {
		searchRecord();
	}
