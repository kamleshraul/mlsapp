				...
				if($("#action").val()=='edit'){
					...
					if(text.length>0){
						if(showIt==0){
							...
							/*.replace(text,'<span style="background: yellow;">'+text+'</span>') */
							...
							//var o=window.open('http://google.com','_blank');
						}
					}else{
						if(showIt==1){
							showIt=0;
							...
							//$('form').attr('action')+'?operation=startworkflow',  
		    	            
		    	            $("#data").val(newContent);
							$.post($("form").attr('action')+'/'+whichId.substring(2),
									$("form").serialize(),function(v){
								if(data=='SUCCESS'){
									showEditProceeding();
								}
							}).fail(function(){
