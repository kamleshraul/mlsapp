...
<head>
	...
	function viewContent(){
		var parameters = "houseType="+$("#assihouseType").val()
		 ...
		 +"&discussionMotionType="+$("#assimotionType").val()
		 ...
		 var resource='discussionmotion/bulksubmission/assistant/view';
	...
	function loadActors(value){
		...
		if(motion!=undefined&&motion!=''){
			var params="discussionmotion="+motion+"&status="+value+
	...
	function bulkPutUpUpdate(){
		...
		var aprstatus=$("#assiInternalStatus").val();
		var status = $("#selectedStatus").val();
		$.prompt($('#submissionMsg').val(),{
			buttons: {Ok:true, Cancel:false}, callback: function(v){
	        if(v){
				$.blockUI({ message: '<img src="./resources/images/waitAnimated.gif" />' });
				$.post('discussionmotion/bulksubmission/assistant/update?actor='+next+"&level="+level,
			        	{items:items
			        	,status:status
			        	,aprstatus:aprstatus
...
<body>	
	...
	<input type="hidden" id="assimotionType" value="${motionType }">
