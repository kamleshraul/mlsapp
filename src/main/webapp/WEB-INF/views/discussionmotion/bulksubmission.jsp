		...
		$("#bulksubmit").click(function(){
			$.prompt($('#submissionMsg').val(),{
		        ...
		        if(v){
		        	...
		        	$.post('discussionmotion/bulksubmission?items='+items
...
<body>	
	...
	<c:choose>
		<c:when test="${!(empty motions) }">
			...
			<table class="uiTable">
				<tr>
					...
					<th><spring:message code="discussionmotion.subject" text="Subject"></spring:message></th>
					<security:authorize access="hasAnyRole('DMOIS_TYPIST')">
						<th><spring:message  code="motion.member" text="Member" /></th>
					</security:authorize>
					<th><spring:message code="generic.up" text="Up"></spring:message></th>
					<th><spring:message code="generic.down" text="Down"></spring:message></th>
				</tr>			
				...
				<c:forEach items="${motions}" var="i">
					<tr id="row${index}">
						...
						<security:authorize access="hasAnyRole('DMOIS_TYPIST')">														
							<td class="member">
								<c:choose> 
									<c:when test="${i.primaryMember.aliasEnabled == true}">
										${i.primaryMember.getFirstNameAliasLastName()} 
									</c:when>
									<c:otherwise>
										${i.primaryMember.getFullName()}
									</c:otherwise>
								</c:choose>
								${lName}
							</td>
		...
		<c:otherwise>
			<spring:message code="motion.generic.nomotions" text="No Completed Motions Found"></spring:message>
		</c:otherwise>
	</c:choose>
