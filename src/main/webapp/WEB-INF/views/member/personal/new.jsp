...
<head>
	...
	$(document).ready(function(){	
		...
		$('#memberType').change(function(){			
			if($('#memberType :selected').val() != "member") {					
				$('#house_tab').hide();
				$('#minister_tab').hide();
				$('#party_tab').hide();
				$('#election_tab').hide();
...
<form:form action="member/personal" method="POST" modelAttribute="domain">
	...
	<p>
		<label class="small"><spring:message code="member.personal.memberType" text="Member Type"/></label>
		<select id="memberType" name="mType" class="sSelect" >
			<c:forEach items="${memberTypes}" var="mtype">
				<option value="${mtype.type}">${mtype.name}</option>
			</c:forEach>
		</select>
		<%-- <form:select path="memberType" items="${memberTypes}" itemValue="type" itemLabel="name" cssClass="sSelect"/> --%>
		<form:errors path="memberType" cssClass="validationError"/>
	</p>
	<p>
