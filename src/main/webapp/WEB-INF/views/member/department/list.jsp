...
<html>
<head>
	<title><spring:message code="member.department.list" text="List of Departments"/></title>
	...
	<script type="text/javascript">
		...
		function newRecord(member){
				$.get('member/department/new?member='+member, function(data){					
		...
		function editRecord(row,member) {			
			...
			$.get('member/department/'+row+'/edit?member='+member, function(data){
				$('#grid_container').html(data);
				$('#list_record').show();					
		});		
		}
		function rowDblClickHandler(rowid, iRow, iCol, e) {
			var member=$('#key').val();
			$.get('member/department/'+rowid+'/edit?member='+member, function(data){
				$('#grid_container').html(data);
				$('#list_record').show();					
		});
		}
		...
		function deleteRecord(row) {
			...
			if(row ==""){
				...
				$.prompt($('#confirmDeleteMessage').val()+ row,{
			        ...
			        if(v){
				        $.delete_('member/department/'+row+'/delete?member='+member, null, function(data, textStatus, XMLHttpRequest) {
				        	listRecord();
				        });
			        }
				}});
