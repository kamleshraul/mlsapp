		...
		<table>
			...
			<c:if test="${selectedQuestionType=='questions_halfhourdiscussion_from_question'}">
				...
				<tr>
					...
					<td>
						<c:set var="hdqRefDev" value="-" />
						<c:forEach items="${hdqRefDevices}" var="d">
							<c:if test="${d==hdqRefDeviceSelected}">
								<c:set var="hdqRefDev" value="${d}" />
							</c:if>
						</c:forEach>
						<c:out value="${hdqRefDev}"></c:out>
					</td>
				</tr>
				...
				<c:when test="${!empty ministries}">
					...
					<c:if test="${selectedQuestionType=='questions_starred'}">
						<tr>
							...
							<td>
								<c:set var="ansDate" value="-" />
								<c:forEach items="${answeringDates }" var="i">
									<c:choose>
										<c:when test="${i.id==answeringDate }">
											<c:set var="ansDate" value="${i.name}" />
										</c:when>
									</c:choose>
								</c:forEach>
								<c:out value="${ansDate}"></c:out>
							</td>
						</tr>
					</c:if>
						...
						<c:when test="${not (selectedQuestionType=='questions_halfhourdiscussion_standalone' and houseTypeType=='lowerhouse')}">
							...
							<c:if test="${selectedQuestionType=='questions_starred'}">
								<tr>
									...
									<td>
										<c:set var="domPriority" value="-" />
										<c:forEach items="${priorities}" var="p">
											<c:if test="${p.number==domain.priority}">
												<c:set var="domPriority" value="${p.name}" />
											</c:if>
										</c:forEach>
										${domPriority}
									</td>	
