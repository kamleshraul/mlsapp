...
<head>
	...
	<script type="text/javascript">
		...
		function newAppropriationBillMotion_ForNew() {
		...
		$(document).ready(function(){					
			...
			/**** SubDepartment Changes (Validate as per Department Priorities of corresponding appropriationbillmotions) ****/
		    ...
		    $("#viewStatus").click(function(){
			    $.get('appropriationbillmotion/status/'+$("#id").val(),function(data){
				    $.fancybox.open(data);
			    }).fail(function(){
					if($("#ErrorMsg").val()!=''){
						$("#error_p").html($("#ErrorMsg").val()).css({'color':'red', 'display':'block'});
			...
			$('#new_record_ForNew').click(function(){	
				newAppropriationBillMotion_ForNew();
			});
...
<body>
	...
	<div class="commandbar">
		<div class="commandbarContent">
			...
			<a href="#" id="new_record_ForNew" class="butSim">
				<spring:message code="appropriationbillmotion.new" text="New"/>
			</a> |
			</security:authorize>
		</div>
	</div>
	<div class="fields clearfix watermark">
		...
		<form:form action="appropriationbillmotion" method="PUT" modelAttribute="domain">
			...
			<div id="reportDiv">
				...
				<c:if test="${!(empty domain.number)}">
					<p>
						<label class="small"><spring:message code="appropriationbillmotion.number" text="AppropriationBillMotion Number"/>*</label>
				...
				<p style="display:none;">
					<label class="small"><spring:message code="appropriationbillmotion.type" text="Type"/>*</label>
			 ...
			 <div class="fields">
				<h2></h2>
				<c:choose>
					<c:when test="${memberStatusType=='appropriationbillmotion_complete' or memberStatusType=='appropriationbillmotion_incomplete'}">
						<p class="tright">
					...
					<c:otherwise>
						<p class="tright">
							<security:authorize access="hasAnyRole('CMOIS_CLERK')">	
								<input id="submitAppropriationBillMotion" type="button" value="<spring:message code='motion.submitmotion' text='Submit Motion'/>" class="butDef" disabled="disabled">				
							</security:authorize>			
							<security:authorize access="hasAnyRole('MEMBER_LOWERHOUSE','MEMBER_UPPERHOUSE')">		
								...
								<input id="submitAppropriationBillMotion" type="button" value="<spring:message code='appropriationbillmotion.submitmotion' text='Submit Motion'/>" class="butDef" disabled="disabled">
		...
		<input id="departmentUnavailableForDiscussionMsg" value="<spring:message code='appropriationbillmotion.departmentUnavailableForDiscussionMsg' text='This Department is unavailable for discussion!'/>" type="hidden">
	</div>
</body>
</html>