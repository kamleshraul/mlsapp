		...
		$("#bulksubmit").click(function(){
			$.prompt($('#submissionMsg').val(),{
		        ...
		        if(v){
		        	...
		        	$.post('proprietypoint/bulksubmission?items='+items
...
<body>	
	...
	<c:choose>
		<c:when test="${!(empty proprietyPoints) }">
			<input type="button" id="bulksubmit" value="<spring:message code='generic.submit' text='Submit'/>"  style="width: 100px;margin: 10px;"/>		
			<table class="uiTable">
				<tr>
					<th style="text-align:center"><spring:message code="proprietypoint.submitall" text="Submit All"></spring:message>
					...
					<th><spring:message code="proprietypoint.up" text="Up"></spring:message></th>
					<th><spring:message code="proprietypoint.down" text="Down"></spring:message></th>
				</tr>			
				<c:set var="index" value="1"></c:set>	
				<c:forEach items="${proprietyPoints}" var="i">
					<tr id="row${index}">
		...
		<c:otherwise>
			<spring:message code="proprietypoint.nodevices" text="No Completed Propriety Points Found"></spring:message>
		</c:otherwise>
	</c:choose>
	<input type="hidden" id="size" value="${size }">	
</div>	
	<input id="submissionMsg" value="<spring:message code='proprietypoint.client.prompt.bulksubmit' text='Do you want to submit the selected propriety points?'></spring:message>" type="hidden">
