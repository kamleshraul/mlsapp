...
<script type="text/javascript">
	$(document).ready(function(){	
		...
		$("#prois_register_admission_number_report").click(function() {
			$(this).attr('href','#');
	...
	function generateRegisterReport() {
		...
		$("#prois_register_admission_number_report").attr('href',
	...
	function bulkUpdateAdmissionNumberChange(){
	    ...
	    $.prompt($('#submissionMsg').val(),{
	        ...
	        if(v){
				...
				$.ajax({url: 'proprietypoint/bulkadmissionpointnumberapproval/update', 
					data: {						
						 items:admission,
...
<a href="#" id="prois_register_admission_number_report" class="butSim">
	<spring:message code="prois.rejected_report" text="Register Report"/>
</a>	
<br/><br/>
<p id="error_p" style="display: none;">&nbsp;</p>
	<c:if test="${(error!='') && (error!=null)}">
	...
	<c:choose>
		...
		<c:otherwise>
			<spring:message code="resolution.noresolutions" text="No Propriety Point Found"></spring:message>				
		</c:otherwise>
	</c:choose>
<%-- 	<input type="hidden" id="resolutionId" value="${resolutionId}"> --%>
	<input id="submissionMsg" value="<spring:message code='proprietypoint.client.prompt.submit' text='Do you want change the admission Number of Propriety Point ?'></spring:message>" type="hidden">
