...
<script type="text/javascript">
	...
	function showCutMotionList() {
		...
		showTabByIdAndUrl('list_tab', 'motion/cutmotion/list?houseType='
	...
	function newCutMotion() {
		...
		//since id of cut motion has not been created so key is set to empty value
		//$("#key").val("");
		$("#newRecordDiv").show();
		$(".tabContent").empty();
		$(".tabContent").show();
		$("#selectionDiv1").hide();
		$("#gridDataSaverDiv").hide();
		showTabByIdAndUrl('details_tab', 'motion/cutmotion/new?' + $("#gridURLParams_ForNew").val());
	}
	...
	function editCutMotion(row) {
		...
		if (row == null || row == '') {
			...
			$("#selectionDiv1").hide();
			$("#gridDataSaverDiv").hide();
			showTabByIdAndUrl('details_tab', 'motion/cutmotion/' + row + '/edit?' + $("#gridURLParams").val());
		}
	}
	...
	function rowDblClickHandler(rowid, iRow, iCol, e) {
		...
		$('#key').val(rowid);
		showTabByIdAndUrl('details_tab', ',motion/cutmotion/' + rowid + '/edit?' + $("#gridURLParams").val());
	}
	/**** delete question ****/
	function deleteCutMotion() {
		...
		if (row == null || row == '') {
			...
			$.prompt($('#confirmDeleteMessage').val() + row, {buttons : {Ok : true,Cancel : false},
				callback : function(v) {
					if (v) {
						$.delete_('motion/cutmotion/' + row+ '/delete',null,function(data, textStatus, XMLHttpRequest) {
							showCutMotionList();
						}).fail(function() {
							if ($("#ErrorMsg").val() != '') {
	...
	function bulkPutup() {
		...
		var resourceURL = 'motion/cutmotion/bulksubmission?' + parameters + "&itemscount=" + $("#selectedItemsCount").val();
		$(".tabContent").empty();
		$(".tabContent").show();
		$("#selectionDiv1").hide();
		$("#gridDataSaverDiv").hide();
	...
	function bulkPutupAssistant() {
		...
		var resourceURL = 'motion/cutmotion/bulksubmission/assistant/int?' + parameters + "&itemscount=" + $("#selectedItemsCount").val();
		$(".tabContent").empty();
		$(".tabContent").show();
		$("#selectionDiv1").hide();
		$("#gridDataSaverDiv").hide();
...
<body>
	<div class="clearfix tabbar">
		...
		<div class="commandbarContent" style="margin-top: 10px;" id="selectionDiv1">
			...
			<a href="#" id="select_session_year" class="butSim"> <spring:message code="generic.sessionyear" text="Year" /></a> 
			...
			<c:choose>
				<c:when test="${cutmotionTypeType=='cutmotions_budgetary'}">
					<a href="#" id="devicetypeLabel" class="butSim"> <spring:message code="cutmotion.cutmotionType" text="Cut Motion Type" /></a>
				</c:when>
				<c:otherwise>
					<a href="#" id="select_cutMotionType" class="butSim"> <spring:message code="cutmotion.cutmotionType" text="Cut Motion Type" />
					</a>
				</c:otherwise>
			</c:choose>
		...
		<div id="newRecordDiv">
			<a href="#" id="new_record_saver" class="butSim"><spring:message code="cutmotion.new" text="New"/></a>
		</div>
			
		<div class="tabContent"></div>

		<div id="gridDataSaverDiv"></div>
