...
<head>
	...
	function clubbingInt(id){
		$.blockUI({ message: '<img src="./resources/images/waitAnimated.gif" />' });
		var params="specialMentionNoticeId="+id
					+"&usergroup="+$("#currentusergroup").val()
			        +"&usergroupType="+$("#currentusergroupType").val();		
	...
	function loadActors(value){
	    ...
	    var clubbingPostAdmission = $("#internalStatusMaster option[value='governorspeechnotice_recommend_clubbingPostAdmission']").text();
		var unclubbing = $("#internalStatusMaster option[value='governorspeechnotice_recommend_unclubbing']").text();
		var admitDueToReverseClubbing = $("#internalStatusMaster option[value='governorspeechnotice_recommend_admitDueToReverseClubbing']").text();
	...
	$(document).ready(function(){
		...
		/**** To show/hide view ClubbedSpecialMentionNoticeTextsDiv to view clubbed special mention notice's text starts****/
		...
		/* $(".clubbedRefMotions").contextMenu({
			...
			if(action=='unclubbing'){
				...
				$.post('clubentity/unclubbing?pId='+motionId+"&cId="+clubId+"&whichDevice=notices_specialmention_"+"&usergroupType="+$("#currentusergroupType").val(),function(data){
				...
				$.post('refentity/governorspeechnotice/dereferencing?pId='+motionId+"&rId="+refId,function(data){
					if(data=='SUCCESS'){
...
<body>
	...
	<div id="assistantDiv">
		<form:form action="governorspeechnotice" method="PUT" modelAttribute="domain">
			...
			<p>
				...
				<c:when test="${internalStatusType=='governorspeechnotice_final_admission'}">
					<label class="small"><spring:message code="governorspeechnotice.admissionNumber" text="Admission Number"/></label>
			...
			<p>
				<label class="centerlabel"><spring:message code="specialmentionnotice.members" text="MemberName"/></label>
		...
		<%-- 	<p>
			...
			<c:if test="${internalStatusType == 'governorspeechnotice_system_assistantprocessed' || internalStatusType == 'governorspeechnotice_putup_rejection'
				|| internalStatusType == 'governorspeechnotice_putup_clubbing' || internalStatusType == 'governorspeechnotice_putup_nameclubbing' 
				|| recommendationStatusType == 'governorspeechnotice_putup_clubbingPostAdmission' || recommendationStatusType == 'governorspeechnotice_putup_clubbingWithUnstarredFromPreviousSession'
				|| recommendationStatusType == 'governorspeechnotice_putup_unclubbing' || recommendationStatusType == 'governorspeechnotice_putup_admitDueToReverseClubbing'
			}">
				<security:authorize access="hasAnyRole('GSN_ASSISTANT','GSN_SECTION_OFFICER')">
				<p>	
			...
			<c:if test="${fn:contains(internalStatusType, 'governorspeechnotice_final')}">
				<form:hidden path="actor"/>
			</c:if>
			
			<%-- <c:choose>
			<c:when test="${internalStatusType eq 'governorspeechnotice_final_admission'}">
			<p>
			...
			<div class="fields">
				...
				<p class="tright">
					<c:choose>
						<c:when test="${(internalStatusType eq'governorspeechnotice_final_admission' || internalStatusType eq 'governorspeechnotice_final_rejection')}">
							<input id="submit" type="submit" value="<spring:message code='generic.submit' text='Submit'/>" class="butDef">
						</c:when>
						<c:when test="${bulkedit!='yes'}">
							<c:if test="${internalStatusType=='governorspeechnotice_submit' || internalStatusType=='governorspeechnotice_system_assistantprocessed' || internalStatusType=='governorspeechnotice_system_putup'
								|| internalStatusType=='governorspeechnotice_putup_rejection'}">
								<input id="submit" type="submit" value="<spring:message code='generic.submit' text='Submit'/>" class="butDef">
								<security:authorize access="hasAnyRole('GSN_ASSISTANT','GSN_SECTION_OFFICER')">
		...
		<input id="startWorkflowMessage" name="startWorkflowMessage" value="<spring:message code='specialmentionnotice.startworkflowmessage' text='Do You Want To Put Up Governor Speech Notice?'></spring:message>" type="hidden">
