...
<html>
<head>
<title><spring:message code="governorspeechnotice" text="Governor Speech Notice"/></title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
	<script type="text/javascript">
		...
		function showSpecialMentionNoticeList() {
			showTabByIdAndUrl('list_tab','governorspeechnotice/list?houseType='+$('#selectedHouseType').val()
		...
		function memberGovernorSpeechNoticeView() {
			...
			+ "&report=MEMBER_GOVERNORSPEECHNOTICE_VIEW"
			+ "&reportout=member_governorspeechnotice_view";
			showTabByIdAndUrl('details_tab','governorspeechnotice/report/generalreport?'+parameters);
		}
		...
		function newSpecialMentionNotice() {		
			...
			$("#key").val("");	
			showTabByIdAndUrl('details_tab','governorspeechnotice/new?'+$("#gridURLParams").val());
			$("#selectionDiv1").hide();	
		}
		...
		function editSpecialMentionNotice(row) {
			...
			if(row==null||row==''){
				...
				showTabByIdAndUrl('details_tab','governorspeechnotice/'+row+'/edit?'+$("#gridURLParams").val());
				$("#selectionDiv1").hide();	
			}			
		}
		...
		function rowDblClickHandler(rowid, iRow, iCol, e) {
			...
			$('#key').val(rowid);			
			showTabByIdAndUrl('details_tab', 'governorspeechnotice/'+rowid+'/edit?'+$("#gridURLParams").val());
		}	
			...
			if(row == null || row == ''){
			    ...
			    if(deviceNumber!='-') {
					...
					$.prompt($('#confirmDeleteMessage').val()+ row,{
				        ...
				        if(v){
					        $.delete_('governorspeechnotice/'+row+'/delete', null, function(data, textStatus, XMLHttpRequest) {
					        	showSpecialMentionNoticeList();
					        }).fail(function(){
								if($("#ErrorMsg").val()!=''){
									$("#error_p").html($("#ErrorMsg").val()).css({'color':'red', 'display':'block'});
		...
		/**** Set Submission Time Window ****/
	 	/* function setSubmissionTimeWindow() {
			...
			if(selectedSpecialMentionNoticeDate==undefined || selectedSpecialMentionNoticeDate=="") {
				...
				+"&usergroup="+$("#currentusergroup").val()
				+"&usergroupType="+$("#currentusergroupType").val();
				var resourceURL='specialmentionnoticesubmissionwindow?'+parameters;
...
<body>
	...
	<div class="clearfix tabbar">
		<ul class="tabs">
			...
			<security:authorize access="hasAnyRole('GSN_CLERK','GSN_ASSISTANT', 'GSN_UNDER_SECRETARY',
			'GSN_DEPUTY_SECRETARY','GSN_PRINCIPAL_SECRETARY','GSN_SPEAKER', 'GSN_JOINT_SECRETARY',
			'GSN_SECRETARY', 'GSN_OFFICER_ON_SPECIAL_DUTY', 'GSN_DEPUTY_SPEAKER', 'GSN_CHAIRMAN',
			'GSN_DEPUTY_CHAIRMAN', 'GSN_SECTION_OFFICER', 'GSN_UNDER_SECRETARY_COMMITTEE',
			'SUPER_ADMIN','GSN_ADDITIONAL_SECRETARY')">
			<li>
		...
		<div class="commandbarContent" style="margin-top: 10px;" id="selectionDiv1">		
			...
			<security:authorize
				access="hasAnyRole('GSN_ADMIN','GSN_ASSISTANT','GSN_UNDER_SECRETARY',
			'GSN_DEPUTY_SECRETARY','GSN_PRINCIPAL_SECRETARY','GSN_SPEAKER','GSN_JOINT_SECRETARY',
			'GSN_SECRETARY','GSN_OFFICER_ON_SPECIAL_DUTY','GSN_DEPUTY_SPEAKER','AMOIS_CHAIRMAN','GSN_DEPUTY_CHAIRMAN',
			'GSN_SECTION_OFFICER','GSN_UNDER_SECRETARY_COMMITTEE','GSN_ADDITIONAL_SECRETARY','GSN_CLERK')">
			...
			<security:authorize access="hasAnyRole('MEMBER_LOWERHOUSE','MEMBER_UPPERHOUSE','GSN_TYPIST')">
			<a href="#" id="select_status" class="butSim">
				<spring:message code="adjournmentmotion.status" text="Status"/>
			</a>
