...
<head>
	<title>
		<spring:message code="prashnavali.list" text="List of Committee Tours"/>
	</title>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>	
	<script type="text/javascript">
	/* =============== ACTIONS =============== */
	function showPrashnavaliList() {
		var id = 'list_tab';
		var url = 'prashnavali/list?' + getGridURLParams();
		showTabByIdAndUrl(id, url);
	}
	
	function newPrashnavali() {
		$('#cancelFn').val('newPrashnavali');
		$('#key').val('');
		var id = 'details_tab';
		var url = 'prashnavali/new?' + getGridURLParams();
		showTabByIdAndUrl(id, url);
	}

	function editPrashnavali(rowId) {
		$('#cancelFn').val('editPrashnavali');
		if(rowId == null || rowId == '') {
		...
		var id = 'details_tab';
		var url = 'prashnavali' + '/' + rowId + '/' + 'edit';
		showTabByIdAndUrl(id, url);
	}

	function deletePrashnavali(rowId) {
		if(rowId == null || rowId == '') {
			var msg = $('#selectRowFirstMessage').val();
			$.prompt(msg);
			return;
		...
		var options = {buttons: {Ok:true, Cancel:false}, 
					   callback: function(v){
			   				if(v){
								$.delete_('prashnavali' + '/' + rowId + '/' + 'delete', 
										null, 
	...
	function searchPrashnavali() {
		searchRecord();
	}

	function viewPrashnavali(rowId) {
		$('#cancelFn').val('viewPrashnavali');
		if(rowId == null || rowId == '') {
			var msg = $('#selectRowFirstMessage').val();
			$.prompt(msg);
			return false;
		...
		var id = 'details_tab';
		var url = 'prashnavali' + '/' + rowId + '/' + 'view';
		showTabByIdAndUrl(id, url);
	}
