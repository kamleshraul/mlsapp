		...
		$("#bulksubmit").click(function(){
			$.prompt($('#submissionMsg').val(),{
		        ...
		        if(v){
		        	...
		        	$.post('eventmotion/bulksubmission?items='+items
...
<body>	
	...
	<c:choose>
		<c:when test="${!(empty motions) }">
			...
			<table class="uiTable">
				<tr>
					...
					<th><spring:message code="eventmotion.up" text="Up"></spring:message></th>
					<th><spring:message code="eventmotion.down" text="Down"></spring:message></th>
				</tr>			
				...
				<c:forEach items="${motions}" var="i">
					<tr id="row${index}">
						...
						<td class="subject">${i.eventTitle}</td>
		...
		<c:otherwise>
			<spring:message code="eventmotion.nomotions" text="No Completed EventMotions Found"></spring:message>
		</c:otherwise>
	</c:choose>
