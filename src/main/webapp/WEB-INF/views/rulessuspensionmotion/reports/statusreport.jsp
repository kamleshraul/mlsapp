...
<head>
	<title>
	<spring:message code="rulessuspensionmotion.statusreport" text="Status Report"/>
	</title>
	...
	<script type="text/javascript">
		...
		function showRemarkReport(){
			...
			if(strLocation=='workflow'){
				...
				selectedRuleSuspensionDate = convertToDbFormat($('#selectedAdjourningDate').val());
			}else if(strLocation=='device'){
				device = $("#selectedMotionType").val();
				if($("#isRuleSuspensitonDateSelected").is(":checked")) {
			...
			'&grid='+ strLocation + 
			'&ruleSuspensionDate='+((selectedRuleSuspensionDate=='' || selectedRuleSuspensionDate==undefined)?'0':selectedRuleSuspensionDate);
			
			$.ajax({
				url: 'ref/pendingtasksdevicesrsmois'+paramVar,
		         async:   false,
		...
		function addRemarkReport(){
			if(ids.length > 0 && counter < ids.length){
				 $.get('rulessuspensionmotion/report/'+ ids[counter] + '/currentstatusreportvm?device='+$("#device").val(),function(data1){
