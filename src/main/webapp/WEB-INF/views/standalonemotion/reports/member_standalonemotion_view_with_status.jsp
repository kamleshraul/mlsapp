...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){			
			 /* $('.editStandaloneMotionFromReport').click(function(){
				 var motionId = $(this).attr('id').split("_")[1];
				 ...
				 showTabByIdAndUrl('details_tab', 'standalonemotion/'+questionId+'/edit?'+parameters);
			 }); */
		});		
	</script>
	 <style type="text/css">
					...
					<table class="strippedTable" border="1" style="width: 750px;">
						<thead>
							<tr>
								...
								<c:choose>
									<c:when test="${report[0][7]=='status_visible'}">
								...
								<c:if test="${report[0][7]=='status_visible'}">
										...
										<c:choose>
											<c:when test="${counter.count > 1}">
												<tr class="page-break-after-forced">
													...
													<c:choose>
														<c:when test="${r[7]=='status_visible'}">
													...
													<c:if test="${r[7]=='status_visible'}">
														<td style="font-size: 12px;vertical-align: top;">
															<c:choose>
																<c:when test="${r[5]=='standalonemotion_final_admission' and (not empty r[9])}">
																	<p align="center">${r[4]}<br/><br/><spring:message code="generic.date" text="Date"/> ${r[9]}</p>
																</c:when>
																<c:when test="${r[5]=='standalonemotion_final_rejection' and (not empty r[8])}">
																	<p align="center">${r[4]}</p>
																	<br/>
																	<p align="justify">${r[8]}</p>
																	<%-- <c:if test="${not empty r[10]}">
																		<p align="justify">${r[10]}</p>
																	</c:if> --%>
																</c:when>
