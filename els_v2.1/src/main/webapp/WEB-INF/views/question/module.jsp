...
<head>
	...
	<script type="text/javascript">
			...
			$("#selectedQuestionType").change(function(){				
				...
				if(text == 'questions_starred' && currentHouseType == 'upperhouse'){
					...
					$("#ballot_tab").hide();
						...
						|| text == 'questions_halfhourdiscussion_from_question' || text == 'questions_halfhourdiscussion_standalone') {
					$("#memberballot_tab").hide();
					$("#ballot_tab").show();
				}
				else {
		...
		function memberStatistics(){
			...
			showTabByIdAndUrl('details_tab','test/memberstatistics?'+$("#gridURLParams").val());
		}
		...
		function viewBallot() {
			...
			if(parameters==undefined){
			 ...
			 parameters = parameters + "&group=" + $("#selectedGroup").val()+"&category=question";
		...
		function bulkPutup(){
			...
			var resourceURL = 'question/bulksubmission?' + parameters +"&itemscount="+$("#selectedItemsCount").val();
			showTabByIdAndUrl('bulkputup_tab', resourceURL);
		}	
		/**** Bulk putup(Assistant)****/
		function bulkPutupAssistant(){
		var parameters=null;
		if($('#currentDeviceType').val()=="questions_halfhourdiscussion_standalone" && $("#selectedHouseType").val()=="lowerhouse"){
			 parameters = "houseType="+$("#selectedHouseType").val()
			 +"&sessionYear="+$("#selectedSessionYear").val()
			 +"&sessionType="+$("#selectedSessionType").val()
			 +"&questionType="+$("#selectedQuestionType").val()
			...
			var resourceURL = 'question/bulksubmission/assistant/int?' + parameters +"&itemscount="+$("#selectedItemsCount").val();
...
<body>				
	<div class="clearfix tabbar">
		<ul class="tabs">
			...
			<security:authorize access="hasAnyRole('QIS_ASSISTANT', 'QIS_UNDER_SECRETARY',
				...
				<security:authorize access="hasAnyRole('QIS_PRINCIPAL_SECRETARY')">
					<li>
					<a id="memberballot_tab" href="#" class="tab" style="display:none;">
				   		<spring:message code="question.memberballot" text="Member Ballot"></spring:message>
					</a>
					</li>
				</security:authorize>	
		...
		<div class="commandbarContent" style="margin-top: 10px;" id="selectionDiv1">
			...
			<c:choose>
				<c:when test="${questionTypeType=='questions_halfhourdiscussion_standalone'}">		
			...
			<security:authorize access="hasAnyRole('MEMBER_LOWERHOUSE','MEMBER_UPPERHOUSE','QIS_CLERK')">
				...
				<security:authorize access="hasAnyRole('MEMBER_LOWERHOUSE','MEMBER_UPPERHOUSE')">	
					...
					<a href="#" id="select_itemcount" class="butSim">
						<spring:message code="motion.itemcount" text="No. of Questions(Bulk Putup)"/>
					</a>
			...
			<security:authorize access="hasAnyRole('QIS_ASSISTANT','HDS_ASSISTANT')">			
					<a href="#" id="select_itemcount" class="butSim">
						<spring:message code="question.itemcount" text="No. of Questions(Bulk Putup)"/>
					</a>
					<select name="selectedItemsCount" id="selectedItemsCount" style="width:100px;height: 25px;">			
				...
				<a href="#" id="select_filecount" class="butSim">
					<spring:message code="question.filecount" text="Select File(Bulk Putup)"/>
				</a>
